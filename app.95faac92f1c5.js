    ...
    // SCA playhead: in the EXPANDED dock the bar-knob IS the play/pause button — a tap toggles,
    // a drag scrubs. In collapsed it's a passive dot, so let the event bubble to the bar (seek).
    var _phDown=null;
    barKnob.addEventListener('pointerdown',function(e){ if(!dock.classList.contains('expanded')||!active) return; e.stopPropagation(); _phDown={x:e.clientX,moved:false}; try{ barKnob.setPointerCapture(e.pointerId); }catch(_){} });
    ...
    function cardHTML(ev){
      ...
      var t1=time12(ev.startTime), t2=time12(ev.endTime);
      var eyebrow = (dayLab||'') + ((dayLab&&t1)?' · ':'') + (t1||'');
      var range = (t1&&t2) ? (t1+'–'+t2) : t1;
      var meta = [esc(ev.venue||'TBA'), ev._city?esc(ev._city):'', range?esc(range):''].filter(Boolean).join(' &middot; ');
      var CAP=4, chips='';
      arts.slice(0,CAP).forEach(function(a){
        var slug=a.contentUrl?a.contentUrl.split('/').filter(Boolean).pop():'';
        chips+='<button class="evc-chip" type="button"'+(slug?' data-slug="'+esc(slug)+'"':'')+'><span class="evc-tri"><svg viewBox="0 0 24 24" fill="currentColor"><path d="M8 5v14l11-7z"/></svg></span>'+esc(a.name)+'</button>';
      });
      if(arts.length>CAP) chips+='<button class="evc-chip more" type="button" data-more>+'+(arts.length-CAP)+' more</button>';
      ...
      return '<article class="gcard evc"'+(eid?' data-evt="'+esc(eid)+'"':'')+'>'+
        '<div class="evc-hero">'+
          ...
          '<div class="evc-over">'+(eyebrow?'<span class="evc-eyebrow">'+esc(eyebrow)+'</span>':'')+'<div class="evc-title">'+esc(ev.title)+'</div>'+(meta?'<div class="evc-meta">'+meta+'</div>':'')+'</div>'+
        ...
        '</div>'+
        (chips?'<div class="evc-body"><div class="evc-chips">'+chips+'</div></div>':'')+
      '</article>';
    }
    ...
    function paintCurrentGrid(){
      ...
      gridEl.querySelectorAll('.gcard').forEach(function(card,i){
        ...
        var mb=card.querySelector('.evc-chip.more'); if(mb) mb.addEventListener('click',function(e){ e.stopPropagation(); open(); });
        // play queue = the whole line-up; the poster-play starts at 0, a chip starts from that artist
