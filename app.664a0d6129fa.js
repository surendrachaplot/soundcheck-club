    ...
    function time12(iso){ var m=/T(\d{2}):(\d{2})/.exec(iso||''); if(!m) return ''; var h=+m[1], mm=m[2], ap=h<12?'am':'pm', x=h%12; return (x||12)+(mm==='00'?'':(':'+mm))+ap; }   // "T23:00" → "11pm" (event-card display)
    ...
    function bumpPlayCount(){ setPref('plays', playCount()+1); }
    function gateForPlay(){
      if(hasEmail()) return Promise.resolve(true);
      if(playCount() < 1) return Promise.resolve(true);   // first one is free
    ...
    function setDockNextGig(slug){
      ...
      function render(g){
        ...
        el.textContent=[ fmtDateLabel(String(g.event_date).slice(0,10)), g.venue, g.city?cityLabel(g.city):'' ].filter(Boolean).join(' · ');
    ...
    // SoundCloud for up to six minutes, then a station-ID voiceover and on to
    // the next. The artist's SoundCloud URL rides along in the day feed, so no
    ...
    function radioStop(){
      ...
      try{ if('speechSynthesis' in window) window.speechSynthesis.cancel(); }catch(_){} // stop any station-ID voice
    ...
    async function radioAdvance(delta){
      ...
      while(tries<radio.queue.length){
        ...
        if(vid){ radio.i=i; radioSayThenPlay(radio.queue[i]); return; }
      }
      showToast('Couldn’t find sets to play.');
      stopActive();
    }
    // Station ID: an auto-generated voice (Web Speech API) instead of canned mp3
    // clips — a short spoken line at the start of the radio and between DJs. The
    // voice is chosen once from the most natural-sounding one the device offers.
    var RADIO_LINES=['Soundcheck Radio', 'Soundcheck'];
    var _scVoice=null, _scVoicePicked=false;
    function pickRadioVoice(){
      if(_scVoicePicked) return _scVoice;
      if(!('speechSynthesis' in window)){ _scVoicePicked=true; return null; }
      var vs=window.speechSynthesis.getVoices()||[];
      if(!vs.length) return null; // not loaded yet — retry on next call / voiceschanged
      _scVoicePicked=true;
      // prefer known natural/premium voices, then any English voice, then anything
      var prefer=[/Google UK English Female/i,/Google US English/i,/Samantha/i,/Serena/i,/Microsoft Aria/i,/Microsoft Jenny/i,/natural/i,/enhanced/i,/premium/i];
      for(var p=0;p<prefer.length;p++){ for(var i=0;i<vs.length;i++){ if(prefer[p].test(vs[i].name)){ _scVoice=vs[i]; return _scVoice; } } }
      for(var j=0;j<vs.length;j++){ if(/^en(-|_|$)/i.test(vs[j].lang)){ _scVoice=vs[j]; return _scVoice; } }
      _scVoice=vs[0]; return _scVoice;
    }
    if('speechSynthesis' in window){ try{ window.speechSynthesis.onvoiceschanged=function(){ _scVoicePicked=false; pickRadioVoice(); }; pickRadioVoice(); }catch(_){} }
    function radioPlayVo(onDone){
      var done=false, fin=function(){ if(done) return; done=true; if(onDone) onDone(); };
      try{
        if(!('speechSynthesis' in window)) return fin();
        var u=new SpeechSynthesisUtterance(RADIO_LINES[Math.floor(Math.random()*RADIO_LINES.length)]);
        var v=pickRadioVoice(); if(v){ u.voice=v; u.lang=v.lang; } else { u.lang='en-GB'; }
        u.rate=0.96; u.pitch=1.0; u.volume=1.0;
        u.onend=fin; u.onerror=fin;
        window.speechSynthesis.cancel();   // clear any stuck/queued utterance
        window.speechSynthesis.speak(u);
        setTimeout(fin, 6000); // safety: never let a stuck voice stall the radio
      }catch(_){ fin(); }
    }
    // Calling this from the start tap also UNLOCKS the audio element on iOS, so
    // the later between-DJ clips (no user gesture) are allowed to play.
    function radioSpeakId(){ radioPlayVo(null); }
    function radioSayThenPlay(entry){
      var started=false, go=function(){ if(started||!radio.on) return; started=true; radioPlayEntry(entry); };
      try{ if(radioYt && radioYt.pauseVideo) radioYt.pauseVideo(); }catch(_){} // duck the outgoing set under the ID
      radioPlayVo(go); // bring in the next set when the clip finishes
    }
    ...
    // it via loadVideoById — which is how advance + the voiceover transitions
    // keep playing without a fresh gesture. Sets play minutes 6→12 (start=360).
    ...
    async function startRadio(){
      ...
      try{
        ...
        radioSpeakId(); // announce on tune-in + unlock speech for the 6-min handoffs (must be in the tap)
        var key=radioWarmKey();
    ...
    async function startEventRadio(ev){
      ...
      if(!(await gateForPlay())) return;
      radioSpeakId(); // announce + unlock speech in the tap
      var q=[];
      (ev.artists||[]).forEach(function(a){
        var slug=a.contentUrl?a.contentUrl.split('/').filter(Boolean).pop():'';
    ...
    document.getElementById('bnav-picks').addEventListener('click',function(){ if(currentUser) openUpcoming(); else openMyPicks(); });
      ...
      if(currentUser){
        ...
        el.innerHTML='<button class="who" id="who-btn" aria-haspopup="true" aria-expanded="false"><span class="av" style="background-image:'+avatarBg(currentUser.handle,currentUser.avatar)+'"></span><span class="hn">u/'+esc(currentUser.handle)+'</span></button>'+
          '<div class="acct-menu" id="acct-menu" role="menu">'+
            ...
            '<button class="am-i" data-am="picks" role="menuitem">My picks</button>'+
    ...
    async function loadMe(){
      ...
      if(currentUser) maybePromptHandle();
      applyPrefCity();
      adoptLegacyLocal();
    }
    ...
    function applyPrefCity(){
      try{
        if(_cityExplicit) return;
        if(prefs.city){ if(prefs.city!==currentCity && typeof setCity==='function') setCity(String(prefs.city)); return; }
        if(_geoTried) return; _geoTried=true;
        fetch(API_BASE+'/api/geo').then(function(r){ return r.json(); }).then(function(j){
          if(j && j.city && j.city!==currentCity && !_cityExplicit && !prefs.city && typeof setCity==='function') setCity(String(j.city));
        }).catch(function(){});
      }catch(_){}
    }
    ...
    function openHandleModal(isRename){
      ...
      go.addEventListener('click',async function(){
        ...
        try{
          ...
          renderAccount(); showToast('You’re u/'+j.handle); close();
    ...
    function openMyPicks(){
      ...
      if(has) h+='<div style="display:flex;gap:9px;margin-bottom:20px;max-width:520px"><button class="btn solid" id="mp-share" type="button" style="flex:1;border:none;border-radius:999px;background:var(--accent);color:var(--accent-ink);font:700 var(--fs-xs) var(--body);letter-spacing:0.08em;text-transform:uppercase;padding:11px;cursor:pointer">Share my picks</button><button class="btn ghost" id="mp-copy" type="button" style="flex:1;background:transparent;color:var(--fg);border:1px solid var(--border);border-radius:999px;font:700 var(--fs-xs) var(--body);letter-spacing:0.08em;text-transform:uppercase;padding:11px;cursor:pointer">Copy link</button></div>';
    ...
    function openAccount(initialTab){
      ...
      Promise.all([
        ...
        var p=res[0]||{}; p.isMe=true; if(!p.handle) p.handle=handle;
    ...
    function pageSkeleton(){
      var line=function(w,h,mb){ return '<div class="sk" style="width:'+w+';height:'+h+'px;border-radius:6px;margin-bottom:'+mb+'px"></div>'; };
    ...
    function feedSkeleton(){
      var row=function(w){ return '<div class="feed-row"><span class="sk" style="flex-shrink:0;width:30px;height:30px;border-radius:999px"></span><div style="flex:1;min-width:0"><span class="sk" style="display:block;width:'+w+';height:13px;border-radius:5px;margin-bottom:7px"></span><span class="sk" style="display:block;width:34%;height:9px;border-radius:4px"></span></div></div>'; };
    ...
    function ptile(slug,name,seen,image){
      var img=image?'<img class="pimg" loading="lazy" decoding="async" src="'+imgUrl(image,300)+'" alt="" onerror="this.remove()">':'';
      return '<button class="ptile" data-artist="'+esc(slug)+'"><span class="ptile-img'+(image?' has-img':'')+'" style="background:'+artGrad(name||slug)+'">'+img+'</span><span class="nm">'+esc(name||slug)+'</span></button>';
    }
    ...
    function sceneStatsHTML(st, mine, seen, handle){
      if(!st || !st.gigs) return '';
      var TKT='<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M2 9a3 3 0 0 1 0 6v2a2 2 0 0 0 2 2h16a2 2 0 0 0 2-2v-2a3 3 0 0 1 0-6V7a2 2 0 0 0-2-2H4a2 2 0 0 0-2 2Z"/><path d="M13 5v2"/><path d="M13 11v2"/><path d="M13 17v2"/></svg>';
      var CHK='<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M22 11.08V12a10 10 0 1 1-5.93-9.14"/><path d="m9 11 3 3L22 4"/></svg>';
      var WAV='<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M2 12h3l2-7 4 16 3-12 2 5h4"/></svg>';
      var BLD='<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M3 21h18"/><path d="M5 21V7l8-4v18"/><path d="M19 21V11l-6-4"/></svg>';
      var USR='<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M16 21v-2a4 4 0 0 0-4-4H6a4 4 0 0 0-4 4v2"/><circle cx="9" cy="7" r="4"/><path d="M22 21v-2a4 4 0 0 0-3-3.87"/><path d="M16 3.13a4 4 0 0 1 0 7.75"/></svg>';
      var venues=st.topVenues||[], genres=st.topGenres||[], seenA=seen||[];
      var sinceYear = st.firstGig ? String(new Date(st.firstGig).getFullYear()) : '';
      var yearTag = "'"+String(new Date().getFullYear()).slice(-2);
      var city=(typeof currentCity!=='undefined'&&currentCity)?String(currentCity):'';
      var serial='SC-'+st.gigs+(city?('-'+city.slice(0,3).toUpperCase()):'');
      var maxN = venues[0] ? venues[0].n : 1;
      var soundsHTML = genres.map(function(g,i){ return '<span class="pc-snd'+(i===0?' pc-lead':'')+'">'+(i===0?WAV:'')+esc(g)+(i===0?' <i>lead</i>':'')+'</span>'; }).join('');
      var venuesHTML = venues.map(function(v,i){
        var pct=Math.max(8, Math.round((v.n/maxN)*100));
        return '<div class="pc-vrow'+(i===0?' pc-top':'')+'"><span class="pc-badge">&times;'+v.n+'</span>'+
          '<div class="pc-vmid"><div class="pc-vname">'+BLD+esc(v.venue)+'</div><div class="pc-meter"><span style="width:'+pct+'%"></span></div></div>'+
          '<div class="pc-vct">'+v.n+' nights</div></div>';
      }).join('');
      var SHOW=8;
      var bubbles = seenA.slice(0,SHOW).map(function(a){
        var bg = a.image ? 'background-image:url('+imgUrl(a.image,120)+')' : 'background:'+artGrad(a.name||a.slug||'');
        return '<span class="pc-bub" data-artist="'+esc(a.slug||'')+'" style="'+bg+'" title="'+esc(a.name||'')+'"></span>';
      }).join('') + (seenA.length>SHOW ? '<span class="pc-bub" style="background:var(--surface-2);display:inline-flex;align-items:center;justify-content:center;font:700 0.64rem var(--head);color:var(--fg)">+'+(seenA.length-SHOW)+'</span>' : '');
      var homeClub = venues[0] ? venues[0].venue : '';
      var strip = (homeClub?'Home club <b>'+esc(homeClub)+'</b>':'') + (homeClub&&sinceYear?'<span class="pc-dot"></span>':'') + (sinceYear?'since <b>'+sinceYear+'</b>':'') + '<span class="pc-sn">'+esc(serial)+'</span>';
      var share = handle ? '<button class="pc-share" type="button" data-taste="'+esc(handle)+'">◆ '+(mine?'Share taste card':'Taste card')+'</button>' : '';
      return '<div class="pc">'+
        '<div class="pc-title">'+TKT+'Your year in music'+share+'</div>'+
        '<div class="pc-stub"><div class="pc-stub-top"><div class="pc-admit">'+TKT+'Admit one</div><div class="pc-seal">'+CHK+'Verified '+yearTag+'</div></div>'+
          '<div class="pc-big"><b>'+st.gigs+'</b><span>nights logged</span></div>'+
          (st.thisYear?'<div class="pc-yr">'+st.thisYear+' this year</div>':'')+
          '<div class="pc-bar"></div></div>'+
        (genres.length?'<div class="pc-block"><div class="pc-lbl">'+WAV+'Sound passport</div><div class="pc-sounds">'+soundsHTML+'</div></div>':'')+
        (venues.length?'<div class="pc-block"><div class="pc-lbl">'+BLD+'Venue stamps</div><div class="pc-venues">'+venuesHTML+'</div></div>':'')+
        (seenA.length?'<div class="pc-block"><div class="pc-lbl">'+USR+'Acts seen <span class="pc-n">'+seenA.length+'<small>artists</small></span></div><div class="pc-bubbles">'+bubbles+'</div></div>':'')+
        '<div class="pc-strip">'+strip+'</div>'+
        '<div class="pc-foot"><span>'+TKT+'Soundcheck passport</span><span>'+(sinceYear?'Building since '+sinceYear:'')+'</span></div>'+
      '</div>';
    }
    // wire a freshly-mounted passport: artist bubbles → artist pages, taste-card pill → taste card
    function wireScene(scope){ var sc=scope||document;
      sc.querySelectorAll('.pc-bub[data-artist]').forEach(function(b){ if(b.dataset.artist) b.addEventListener('click',function(){ openArtist(b.dataset.artist); }); });
      sc.querySelectorAll('.pc-share[data-taste]').forEach(function(b){ b.addEventListener('click',function(e){ e.stopPropagation(); openTasteCard(b.dataset.taste); }); });
    }
    // ── passport profile DASHBOARD: cover + year-in-music card + Wanna see / Seen live + Gig diary ──
    ...
    function pdBub(a, big){ return a&&a.image ? 'background-image:url('+imgUrl(a.image,big?200:90)+')' : 'background:'+artGrad((a&&(a.name||a.slug))||''); }
    function pdCoverHTML(p, s, diary){
      var name=esc(p.displayName||('u/'+p.handle));
      var gigs=(p.stats&&p.stats.gigs)||diary.length||0;
      var city=(typeof currentCity!=='undefined'&&currentCity)?String(currentCity):'';
      var serial='SC-'+gigs+(city?('-'+city.slice(0,3).toUpperCase()):'');
      var actions=p.isMe ? '<button class="pd-gear" id="pd-gear" type="button" aria-label="Settings">'+GEAR_SVG+'</button>'
        : '<button class="pd-follow follow-btn'+(p.isFollowing?' following':'')+'" id="follow-btn" type="button" data-handle="'+esc(p.handle)+'">'+(p.isFollowing?'Following':'Follow')+'</button>';
      return '<section class="pd-cover"><div class="pd-cover-edge"></div><div class="pd-cover-body">'+
        '<span class="pd-avatar" style="background-image:'+avatarBg(p.handle,p.avatar)+';background-size:cover;background-position:center"></span>'+
        '<div class="pd-holder"><p class="pd-kicker">Soundcheck · gig passport</p><h1 class="pd-name">'+name+'</h1><p class="pd-handle">@'+esc(p.handle)+'</p>'+
          '<ul class="pd-stats"><li><b>'+s.seen.length+'</b> seen</li><li><b>'+s.wannaSee.length+'</b> wanna see</li><li><b>'+gigs+'</b> gigs</li><li><b>'+(p.followers||0)+'</b> followers</li></ul></div>'+
        '<div class="pd-cover-actions"><span class="pd-serial">'+esc(serial)+'</span>'+actions+'</div>'+
      '</div></section>';
    }
    function pdStampTiles(artists, n){ return (artists||[]).slice(0,n).map(function(a){ return '<div class="pd-stamp" data-artist="'+esc(a.slug||'')+'"><span class="pd-bub" style="'+pdBub(a,true)+'"></span><span class="pd-sname">'+esc(a.name||a.slug)+'</span></div>'; }).join(''); }
    function pdShelfCardHTML(title, artists, key, icon){
      var SHOW=24, list=artists||[], more=list.length>SHOW?'<div class="pd-card-foot"><button class="pd-more" type="button" data-more="'+key+'">See more <i class="ti ti-arrow-right" aria-hidden="true"></i></button></div>':'';
      return '<section class="pd-card pd-shelf-'+key+'"><div class="pd-card-head"><h2 class="pd-title">'+esc(title)+' <span class="pd-cnt">×'+list.length+'</span></h2><i class="ti '+icon+'" aria-hidden="true" style="color:var(--muted);font-size:1.05rem"></i></div><div class="pd-perf"></div><div class="pd-stamps" id="pd-stamps-'+key+'">'+(list.length?pdStampTiles(list,SHOW):'<div class="empty" style="padding:6px 2px">Nothing yet.</div>')+'</div>'+more+'</section>';
    }
    function pdDiaryCardHTML(diary, isMe){
      diary=diary||[];
      var loggig=isMe?'<button class="pd-loggig" type="button" id="pd-loggig">＋ Log a gig</button>':'<span class="pd-serial">Logged · '+(diary.length)+'</span>';
      var PREV=6;
      var stubs=diary.slice(0,PREV).map(function(d){
        var dt=d.gig_date?new Date(String(d.gig_date).slice(0,10)+'T12:00:00'):null;
        var day=dt?dt.getDate():'–', mon=dt?dt.toLocaleDateString('en-GB',{month:'short'}):'';
        var yr=dt?String(dt.getFullYear()).slice(-2):'';
        var arts=(typeof diaryValid==='function')?diaryValid(d):(d.artists||[]);
        var MAXA=3, shownA=arts.slice(0,MAXA), extraA=arts.length-shownA.length;
        var acts=shownA.map(function(a){ return '<span class="pd-achip" data-artist="'+esc(a.slug||'')+'"><span class="pd-abub" style="'+pdBub(a,false)+'"></span>'+esc(a.name||a.slug)+'</span>'; }).join('')+(extraA>0?'<span class="pd-amore">+'+extraA+'</span>':'');
        var vn=d.venue?'<span class="pd-venue" data-venue="'+esc(d.venue)+'">'+esc(d.venue)+'</span>':'<span class="pd-venue">Gig</span>';
        // date lives in the salmon stub (day/mo · 'yy) — so the body row gives the venue full width
        return '<article class="pd-stub"><div class="pd-stub-stub"><span class="pd-stub-dno">'+day+'</span><span class="pd-stub-mo">'+esc(mon)+(yr?" &rsquo;"+yr:'')+'</span></div><div class="pd-stub-body"><div class="pd-stub-top">'+vn+'</div>'+(acts?'<div class="pd-acts">'+acts+'</div>':'')+'</div></article>';
      }).join('');
      var foot = diary.length>PREV ? '<div class="pd-card-foot"><button class="pd-more" type="button" id="pd-diary-all">See all '+diary.length+' gigs <i class="ti ti-arrow-right" aria-hidden="true"></i></button></div>' : '';
      return '<section class="pd-card pd-diary"><div class="pd-card-head"><h2 class="pd-title">Gig diary <span class="pd-cnt">×'+diary.length+'</span></h2>'+loggig+'</div><div class="pd-perf"></div><div class="pd-stubs">'+(stubs||'<div class="empty" style="padding:6px 2px">No gigs logged yet.</div>')+'</div>'+foot+'</section>';
    }
    function profileDashHTML(p){
      var s=p.shelves||{wannaSee:[],seen:[],events:[],venues:[]}, st=p.stats||{}, diary=p.diary||[];
      var extras='';
      if(s.events&&s.events.length) extras+='<div id="pd-events" class="pd-extra"></div>';
      if(s.venues&&s.venues.length) extras+='<div id="pd-venues" class="pd-extra"></div>';
      return '<div class="pd-page">'+pdCoverHTML(p,s,diary)+
        '<div class="pd-grid"><div class="pd-scene">'+sceneStatsHTML(st,p.isMe,s.seen,p.handle)+'</div>'+
        '<div class="pd-right"><div class="pd-shelves">'+pdShelfCardHTML('Wanna see',s.wannaSee,'wanna','ti-bookmark')+pdShelfCardHTML('Seen live',s.seen,'seen','ti-rosette')+'</div>'+
        pdDiaryCardHTML(diary,p.isMe)+'</div></div>'+extras+'</div>';
    }
    function wireProfileDash(p){
      var sc=panelBody||document, s=p.shelves||{};
      wireProfile(); wireScene(sc);
      sc.querySelectorAll('.pd-stamp[data-artist]').forEach(function(b){ if(b.dataset.artist) b.addEventListener('click',function(){ openArtist(b.dataset.artist); }); });
      sc.querySelectorAll('.pd-achip[data-artist]').forEach(function(b){ if(b.dataset.artist) b.addEventListener('click',function(e){ e.stopPropagation(); openArtist(b.dataset.artist); }); });
      sc.querySelectorAll('.pd-venue[data-venue]').forEach(function(b){ if(b.dataset.venue) b.addEventListener('click',function(){ openVenue(b.dataset.venue); }); });
      var lg=document.getElementById('pd-loggig'); if(lg) lg.addEventListener('click',function(){ openLog(); });
      var da=document.getElementById('pd-diary-all'); if(da) da.addEventListener('click',function(){ openDiaryPage(p.handle, p.isMe); });
      var gear=document.getElementById('pd-gear'); if(gear) gear.addEventListener('click',function(){ openActionSheet({ title:'Settings', items:[
      ...
      sc.querySelectorAll('.pd-more[data-more]').forEach(function(b){ b.addEventListener('click',function(){ var key=b.dataset.more, arts=key==='wanna'?s.wannaSee:s.seen, box=document.getElementById('pd-stamps-'+key); if(box){ box.innerHTML=pdStampTiles(arts, arts.length); box.querySelectorAll('.pd-stamp[data-artist]').forEach(function(t){ t.addEventListener('click',function(){ openArtist(t.dataset.artist); }); }); } var f=b.closest('.pd-card-foot'); if(f) f.remove(); }); });
      var ev=document.getElementById('pd-events'); if(ev && s.events && s.events.length) mountEventRail(ev, s.events.map(function(e){ return {id:e.id,title:e.title,venue:e.venue,event_date:e.day||e.event_date,poster_url:e.poster_url,lineup:e.lineup}; }), {title:'Saved events',count:s.events.length,mt:'24px'});
      var vn=document.getElementById('pd-venues'); if(vn && s.venues && s.venues.length) mountVenueRail(vn, s.venues.map(function(v){ return {venue:v.name,venue_slug:v.slug,image:v.image}; }), {title:'Following venues',count:s.venues.length,mt:'24px'});
    }
    ...
    function diaryTLHTML(diary, isMe){
      if(!diary.length) return '<div class="empty">No gigs logged yet.</div>';
      return '<div class="prof-tl">'+diary.map(function(d,i){
        var dl=diaryDateLabel(d), arts=diaryValid(d);
        var vn = d.venue ? '<button class="dv-venue" type="button" data-venue="'+esc(d.venue)+'">'+esc(d.venue)+'</button>' : 'Gig';
        var tiles = arts.length ? '<div class="ptiles ptiles-wide" style="margin-top:11px">'+arts.map(function(a){ return ptile(a.slug, a.name||a.slug, false, a.image); }).join('')+'</div>' : '';
        var del = (isMe && d.id!=null) ? '<button class="dv-del" type="button" data-del="'+esc(String(d.id))+'" aria-label="Remove gig">&times;</button>' : '';
        return '<div class="row"><div class="spine"><div class="dot"></div>'+(i<diary.length-1?'<div class="ln"></div>':'')+'</div><div class="card">'+del+'<div class="vn">'+vn+'</div>'+(dl?'<div class="mt">'+esc(dl)+'</div>':'')+tiles+(d.note?'<div class="nt">'+esc(d.note)+'</div>':'')+'</div></div>';
      }).join('')+'</div>';
    }
    ...
    function tasteCardHTML(p){
      ...
      var city=st.homeCity||((typeof currentCity!=='undefined'&&currentCity&&p.isMe)?String(currentCity):'');
      var code=tcCityCode(city), since=st.firstGig?String(new Date(st.firstGig).getFullYear()):'';
      var gigs=st.gigs||diary.length||0, thisY=st.thisYear||0;
      var genres=st.topGenres||[], venues=st.topVenues||[], lead=genres[0]||'', hv=venues[0];
      var serial='SC-'+gigs+(code&&code!=='•••'?('-'+code):'');
      ...
      return '<div class="tb-pass">'+
        '<div class="tb-main">'+
          '<div class="tb-topbar"><div class="tb-brand"><div class="tb-logo">S</div><div class="tb-brandtxt"><b>soundcheck</b><span>Boarding Pass · '+yr+' Season</span></div></div><div class="tb-class">Admit One · Resident</div></div>'+
          '<div class="tb-pax"><div class="tb-paxL"><span class="tb-lbl">Passenger</span><div class="tb-name">'+name+'</div><div class="tb-handle">@<b>'+esc(handle)+'</b>'+(city?' · '+esc(city):'')+(since?' · member since '+since:'')+'</div></div><span class="tb-avatar" style="background-image:'+avbg+';background-size:cover;background-position:center"></span></div>'+
          '<div class="tb-route"><div class="tb-node"><div class="tb-code">'+esc(code)+'</div><div class="tb-sub">Home base</div></div><div class="tb-leg"><div class="tb-plane">'+TC_PLANE+'</div></div><div class="tb-node"><div class="tb-code tb-lead">'+esc((lead||'—').toUpperCase())+'</div><div class="tb-sub">Lead genre</div></div></div>'+
          (hv?'<div class="tb-venue"><span class="tb-lbl">Gate · Home venue</span><div class="tb-val">'+esc(hv.venue)+' <small>×'+hv.n+'</small></div><span class="tb-vmeta">Top room</span></div>':'')+
          (seenA.length?'<div class="tb-artists"><div class="tb-arth"><span class="tb-lbl">Seen live</span><span class="tb-artn">'+seenA.length+' acts</span></div><div class="tb-abubs">'+bubbles+'</div></div>':'')+
          '<div class="tb-foot"><div class="tb-qrwrap"><div class="tb-qr"><img class="tb-qr-img" src="'+qr+'" alt="Scan to follow @'+esc(handle)+' on soundcheck" loading="lazy"></div><div class="tb-qrtxt"><span class="tb-lbl">Scan to follow</span><b>Join soundcheck</b><span class="tb-qrurl">/u/'+esc(handle)+'</span></div></div><div class="tb-stamp"><div class="tb-stampin"><span class="tb-tick">'+TC_CHECK+'</span><b>VERIFIED</b><span>Resident</span></div></div></div>'+
        '</div>'+
        '<div class="tb-perf"></div>'+
        '<div class="tb-stub"><span class="tb-lbl">Nights logged · '+yr+' season</span><div class="tb-bignum">'+gigs+'</div><div class="tb-bignum-u">Nights'+(thisY?' · '+thisY+' this year':'')+'</div>'+
          '<div class="tb-stubrow"><div class="tb-stubcol"><div class="tb-sub">Wanna see</div><b>'+(s.wannaSee||[]).length+'</b></div><div class="tb-stubcol"><div class="tb-sub">Seen live</div><b>'+seenA.length+'</b></div><div class="tb-stubcol"><div class="tb-sub">Venues</div><b>'+(s.venues||[]).length+'</b></div></div>'+
          '<div class="tb-serial"><span>Serial no.</span>'+esc(serial)+'</div>'+
        '</div>'+
      '</div>';
    }
    function pbNudgeHTML(p){
      var st=p.stats||{}, diary=p.diary||[], gigs=st.gigs||diary.length||0;
      var target=Math.max(5, Math.ceil((gigs+1)/5)*5), rem=target-gigs, pct=Math.max(4,Math.round(gigs/target*100));
      return '<div class="pb-nudge"><div class="pb-nudge-ic"><i class="ti ti-sparkles" aria-hidden="true"></i></div><div class="pb-nudge-tx"><b>Keep your card growing</b> <span>— log '+rem+' more '+(rem===1?'gig':'gigs')+' to reach '+target+' nights</span></div><div class="pb-meter"><i style="width:'+pct+'%"></i></div><div class="pb-meter-n">'+gigs+' / '+target+'</div></div>';
    }
    function pbStatTiles(p){
      var s=p.shelves||{}, st=p.stats||{}, diary=p.diary||[];
      function tile(key,n,icon,label,sub,on){ return '<button class="pb-tile'+(on?' on':'')+'" type="button" data-pbtab="'+key+'"><div class="pb-tile-n">'+n+'</div><div class="pb-tile-l"><i class="ti '+icon+'" aria-hidden="true"></i> '+label+'</div><div class="pb-tile-sub">'+sub+'</div></button>'; }
      return '<div class="pb-tiles">'+
        tile('diary', st.gigs||diary.length||0,'ti-ticket','Gig diary',(st.thisYear||0)+' this year', true)+
        tile('seen',(s.seen||[]).length,'ti-eye','Seen live','Artists you caught', false)+
        tile('wanna',(s.wannaSee||[]).length,'ti-heart','Wanna see','On your list', false)+
        tile('venues',(s.venues||[]).length,'ti-building-arch','Venues','Followed', false)+
      '</div>';
    }
    function pbStubHTML(d){
      ...
      return '<div class="pb-ds"'+(d.venue?' data-venue="'+esc(d.venue)+'"':'')+'><div class="pb-ds-date"><div class="pb-ds-d">'+day+'</div><div class="pb-ds-m">'+esc(mon)+'</div></div><div class="pb-ds-body"><div class="pb-ds-venue">'+(d.venue?esc(d.venue):'Gig')+'</div><div class="pb-ds-line">'+esc(names||'—')+'</div></div></div>';
    }
    function pbDiaryPanel(p){
      var diary=p.diary||[], isMe=p.isMe, gigs=(p.stats&&p.stats.gigs)||diary.length||0;
      var head='<div class="pb-p-head"><span class="pb-p-title">Gig diary</span><span class="pb-p-count">'+Math.min(diary.length,6)+' OF '+gigs+' STUBS</span></div>';
      var prompt=isMe?'<button class="pb-prob" id="pb-quicklog" type="button"><div class="pb-nudge-ic"><i class="ti ti-map-pin" aria-hidden="true"></i></div><div class="pb-prob-tx"><b>Been to a gig?</b><p>Add the night — it grows your card &amp; diary.</p></div><span class="pb-prob-btn">Log a gig</span></button>':'';
      var stubs=diary.slice(0,6).map(pbStubHTML).join('')||'<div class="empty" style="padding:8px 2px">No gigs logged yet.</div>';
      var more=diary.length>6?'<button class="pb-addrow" id="pb-seeall" type="button"><i class="ti ti-arrow-right" aria-hidden="true"></i> See all '+diary.length+' gigs</button>':(isMe&&diary.length?'<button class="pb-addrow" id="pb-addrow" type="button"><i class="ti ti-plus" aria-hidden="true"></i> Log a gig you went to</button>':'');
      return head+prompt+'<div class="pb-stubs">'+stubs+more+'</div>';
    }
    function pbShelfPanel(p, key){
      var s=p.shelves||{}, seen=(key==='seen'), list=seen?(s.seen||[]):(s.wannaSee||[]), CAP=24;
      var head='<div class="pb-p-head"><span class="pb-p-title">'+(seen?'Seen live':'Wanna see')+'</span><span class="pb-p-count">'+list.length+' ARTISTS</span></div>';
      if(!list.length) return head+'<div class="empty" style="padding:8px 2px">Nothing here yet.</div>';
      var tiles=list.slice(0,CAP).map(function(a){ return ptile(a.slug,a.name,seen,a.image); }).join('');
      var more=list.length>CAP?'<button class="shelf-more" type="button" data-pbmore="'+key+'">See more <span class="sm-n">'+(list.length-CAP)+'</span></button>':'';
      return head+'<div class="ptiles ptiles-wide" id="pb-grid-'+key+'">'+tiles+'</div>'+more;
    }
    function pbVenuesPanel(p){
      var s=p.shelves||{}, nv=(s.venues||[]).length, ne=(s.events||[]).length;
      var head='<div class="pb-p-head"><span class="pb-p-title">Venues &amp; saved</span><span class="pb-p-count">'+nv+' FOLLOWED</span></div>';
      if(!nv && !ne) return head+'<div class="empty" style="padding:8px 2px">No followed venues or saved events yet.</div>';
      return head+'<div id="pb-venues-rail"></div><div id="pb-events-rail"></div>';
    }
    function pbShareCard(p){
      var url='https://soundcheck.club/u/'+p.handle, title=(p.displayName||('u/'+p.handle))+' on soundcheck';
      if(navigator.share){ navigator.share({ title:title, text:'My gig passport on soundcheck', url:url }).catch(function(){}); return; }
      if(navigator.clipboard&&navigator.clipboard.writeText){ navigator.clipboard.writeText(url).then(function(){ showToast('Link copied — your card lives at /u/'+p.handle); }).catch(function(){ showToast(url); }); return; }
      showToast(url);
    }
    function pbProfileHTML(p){
      var isMe=p.isMe;
      var actions=isMe?'<button class="pb-ghost" id="pb-edit" type="button"><i class="ti ti-settings" aria-hidden="true"></i> Edit</button>'
        :'<button class="pb-ghost follow-btn'+(p.isFollowing?' following':'')+'" id="follow-btn" type="button" data-handle="'+esc(p.handle)+'"><i class="ti ti-user-plus" aria-hidden="true"></i> '+(p.isFollowing?'Following':'Follow')+'</button>';
      var shareLabel=isMe?'Share your card':'Share '+esc(p.displayName||('u/'+p.handle))+'’s card';
      return '<div class="pb-root">'+
        '<div class="pb-cover"><div class="pb-brand"><b>soundcheck</b> · passport</div><div class="pb-cover-actions"><button class="pb-ghost" id="pb-followers" type="button"><i class="ti ti-users" aria-hidden="true"></i> '+(p.followers||0)+' followers</button>'+actions+'</div></div>'+
        '<div class="pb-hero">'+tasteCardHTML(p)+
          '<div class="pb-sharebar"><button class="pb-share" id="pb-share" type="button"><span class="pb-ic"><i class="ti ti-share-2" aria-hidden="true"></i></span><span><span class="pb-share-t">'+shareLabel+'</span><span class="pb-share-s">screenshot-ready · scan to follow · '+(new Date().getFullYear())+' edition</span></span></button>'+
          (isMe?'<button class="pb-log" id="pb-log" type="button"><span class="pb-plus">+</span> Log a gig</button>':'')+'</div>'+
          (isMe?pbNudgeHTML(p):'')+
        '</div>'+
        '<div class="pb-side">'+pbStatTiles(p)+'<div class="pb-panels" id="pb-panels"></div></div>'+
      '</div>';
    }
    function wirePbProfile(p){
      var sc=panelBody||document, s=p.shelves||{};
      wireProfile();
      sc.querySelectorAll('.tb-abub[data-artist]').forEach(function(b){ if(b.dataset.artist) b.addEventListener('click',function(){ openArtist(b.dataset.artist); }); });
      var sh=document.getElementById('pb-share'); if(sh) sh.addEventListener('click',function(){ pbShareCard(p); });
      var lg=document.getElementById('pb-log'); if(lg) lg.addEventListener('click',function(){ openLog(); });
      ...
      var panels=document.getElementById('pb-panels');
      function wirePanel(key){
        panels.querySelectorAll('.ptile[data-artist]').forEach(function(b){ b.addEventListener('click',function(){ openArtist(b.dataset.artist); }); });
        panels.querySelectorAll('.pb-ds[data-venue]').forEach(function(b){ b.addEventListener('click',function(){ if(b.dataset.venue) openVenue(b.dataset.venue); }); });
        var ql=document.getElementById('pb-quicklog'); if(ql) ql.addEventListener('click',function(){ openLog(); });
        var ar=document.getElementById('pb-addrow'); if(ar) ar.addEventListener('click',function(){ openLog(); });
        var sa=document.getElementById('pb-seeall'); if(sa) sa.addEventListener('click',function(){ openDiaryPage(p.handle, p.isMe); });
        panels.querySelectorAll('[data-pbmore]').forEach(function(b){ b.addEventListener('click',function(){ var k=b.dataset.pbmore, list=k==='seen'?s.seen:s.wannaSee, grid=document.getElementById('pb-grid-'+k); if(grid){ grid.innerHTML=list.map(function(a){ return ptile(a.slug,a.name,k==='seen',a.image); }).join(''); grid.querySelectorAll('.ptile[data-artist]').forEach(function(t){ t.addEventListener('click',function(){ openArtist(t.dataset.artist); }); }); } b.remove(); }); });
        if(key==='venues'){
          var vr=document.getElementById('pb-venues-rail'); if(vr&&s.venues&&s.venues.length) mountVenueRail(vr, s.venues.map(function(v){ return { venue:v.name, venue_slug:v.slug, image:v.image }; }), { title:'Following venues', count:s.venues.length });
          var er=document.getElementById('pb-events-rail'); if(er&&s.events&&s.events.length) mountEventRail(er, s.events.map(function(e){ return { id:e.id, title:e.title, venue:e.venue, event_date:e.day||e.event_date, poster_url:e.poster_url, lineup:e.lineup }; }), { title:'Saved events', count:s.events.length, mt:'18px' });
        }
      }
      function paint(key){
        if(key==='diary') panels.innerHTML=pbDiaryPanel(p);
        else if(key==='seen'||key==='wanna') panels.innerHTML=pbShelfPanel(p,key);
        else panels.innerHTML=pbVenuesPanel(p);
        wirePanel(key);
      }
      sc.querySelectorAll('.pb-tile[data-pbtab]').forEach(function(t){ t.addEventListener('click',function(){ sc.querySelectorAll('.pb-tile').forEach(function(x){ x.classList.toggle('on', x===t); }); paint(t.dataset.pbtab); }); });
      paint('diary');
    }
    async function openProfile(handle){
      ...
      try{
        ...
        armSticky(p.displayName||('u/'+p.handle), profPlay, (panelBody&&panelBody.querySelector('.pb-hero'))||document.getElementById('page-body'));
    ...
    async function openDiaryPage(handle, isMe){
      ...
      try{
        ...
        var logBtn = mine ? '<span class="more" id="diary-log">+ Log a gig</span>' : '';
        ...
        var body='<div class="pd-page diary-page" style="max-width:760px">'+bc+
          '<div class="sec-h" style="margin-top:4px">Gig diary<span class="ct">'+diary.length+'</span>'+logBtn+'</div>'+
          diaryTLHTML(diary, mine)+'</div>';
        openPanel(body);
        wireDiary(panelBody, mine, function(){ openDiaryPage(handle, mine); });
        var lg=document.getElementById('diary-log'); if(lg) lg.addEventListener('click',function(){ openLog(); });
        ...
        armSticky((p.displayName||('u/'+handle))+' · gig diary', play, (panelBody&&panelBody.querySelector('.sec-h'))||document.getElementById('page-body'));
    ...
    function mountEventCards(el, rows){
      ...
      var evs=(rows||[]).map(function(g){
        return { id:g.id, title:g.title, venue:g.venue,
    ...
    function railSkeleton(el, opts){
      ...
      for(var i=0;i<n;i++){
        if(k==='event') t+='<div class="srail-sk-t" style="width:142px"><div class="sk" style="width:142px;aspect-ratio:3/4;border-radius:11px"></div><div class="sk srail-sk-l" style="width:80%"></div><div class="sk srail-sk-l" style="width:55%"></div></div>';
        else if(k==='venue') t+='<div class="srail-sk-t"><div class="sk" style="width:208px;height:124px;border-radius:13px"></div></div>';
    ...
    function mountEventRail(el, items, opts){
      ...
      el.innerHTML=secH(opts.title, opts.mt, opts.count!=null?('<span class="ct">'+esc(String(opts.count))+'</span>'):'')+'<div class="srail srail-anim">'+items.map(function(e){
        ...
        return '<div class="srail-post" data-ev="'+esc(String(e.id))+'"><div class="srail-post-img" style="'+img+'">'+match+gem+((e.lineup&&e.lineup.length)?'<span class="srail-play br" data-evplay="'+esc(String(e.id))+'" aria-label="Play line-up">&#9654;</span>':'')+'</div>'+
          '<div class="srail-post-t">'+esc(ttl)+'</div><div class="srail-post-s">'+esc([e.venue,d].filter(Boolean).join(' · '))+'</div></div>';
      }).join('')+'</div>';
    ...
    function mountVenueRail(el, items, opts){
      ...
      el.innerHTML=secH(opts.title, opts.mt, opts.count!=null?('<span class="ct">'+esc(String(opts.count))+'</span>'):'')+'<div class="srail srail-anim">'+items.map(function(v){
        ...
        return '<div class="srail-ven" data-venue="'+esc(v.venue)+'" style="'+img+'"><span class="srail-ven-n">'+esc(v.venue)+'</span><span class="srail-play br" data-vplay="'+esc(v.venue)+'" aria-label="Play next night at '+esc(v.venue)+'">&#9654;</span></div>';
      }).join('')+'</div>';
      el.querySelectorAll('.srail-ven[data-venue]').forEach(function(c){ c.addEventListener('click',function(e){ if(e.target.closest('.srail-play')) return; openVenue(c.dataset.venue, city); }); });
      el.querySelectorAll('.srail-play[data-vplay]').forEach(function(b){ b.addEventListener('click',function(e){ e.stopPropagation(); playVenueNextYt(b.dataset.vplay, city); }); });
    }
    ...
    function openSearch(){
      ...
      showPage('<h2 class="page-title">Search</h2><div class="sub" style="margin-bottom:14px">Find artists, venues, events and people across '+esc(cityMeta().name)+'.</div>'+
    ...
    function renderSearch(el, j, q){
      ...
      if(ven.length) mountVenueRail(document.getElementById('sx-ven-s'), ven.map(function(v){ return { venue:v.name, venue_slug:v.slug }; }), {title:'Venues', count:ven.length, city:currentCity, mt:'22px'});
      if(ev.length) mountEventRail(document.getElementById('sx-evt-s'), ev, {title:'Events', count:ev.length, mt:'22px'});
    }
    ...
    async function openArtist(slug){
      ...
      try{
        ...
        if((a.alongside||[]).length){
          h+='<div class="sec-h" style="margin-top:26px">'+
            ...
            '</div>'+
            '<p style="color:var(--muted);font:500 var(--fs-sm) var(--body);margin:6px 0 12px">Artists who share '+esc(a.name||slug)+'’s lineups, promoters &amp; crowd. Hit play for a radio of all of them.</p>'+
            '<div id="similar-rail"></div>';
        }
        h+='<div id="ap-upcoming"></div>';
        // Past sets — the gig-history timeline.
        if((a.history||[]).length){
          h+='<div class="sec-h" style="margin-top:26px">Past sets</div><ul class="gig-history">'+a.history.map(function(g){
            var d=fmtDateLabel(String(g.event_date).slice(0,10));
            var label=g.title||g.venue||'Event';
            var sub=(g.title&&g.venue)?g.venue:cityLabel(g.city);
        ...
        var rows=(a.upcoming||[]).map(function(g){
          return {
            id:g.id, title:g.title, venue:g.venue,
    ...
    async function openPromoter(slug){
      ...
      try{
        ...
        if(ppst && pst.length){
          ppst.innerHTML='<div class="sec-h" style="margin-top:26px">Past events</div><ul class="gig-history">'+pst.map(function(g){
            var d=fmtDateLabel(String(g.event_date).slice(0,10));
            var line=(g.lineup||[]).map(function(a){ return a.name; }).filter(Boolean).slice(0,3).join(', ');
            var label=g.title||line||'Event';
            return '<li class="gh-row" data-open-evt="'+esc(String(g.id))+'"><span class="gh-date">'+esc(d)+'</span>'+
              '<button class="gh-venue" type="button" data-open-evt="'+esc(String(g.id))+'">'+esc(label)+(g.venue?' <small>'+esc(g.venue)+'</small>':'')+'</button></li>';
          }).join('')+'</ul>';
        ...
        if(pa && arts.length){
          pa.innerHTML='<div class="sec-h" style="margin-top:26px">Artists they book</div><div class="ptiles ptiles-wide">'+arts.map(function(x){ return ptile(x.slug,x.name,false,x.image_url); }).join('')+'</div>';
        ...
        if(pv && vns.length){
          pv.innerHTML='<div class="sec-h" style="margin-top:26px">Venues they use</div><div class="pp-tags">'+vns.map(function(v){ return '<span class="sc-tag">'+esc(v.venue)+' <small>'+v.n+'</small></span>'; }).join('')+'</div>';
        }
        var sims=(p.similar||[]), ps=document.getElementById('pp-similar');
        if(ps && sims.length){
          ps.innerHTML='<div class="sec-h" style="margin-top:26px">Similar promoters</div><div class="pp-tags">'+sims.map(function(s){ return '<button class="pp-prom" type="button" data-promoter="'+esc(s.slug)+'">'+esc(s.name)+' <small>'+s.shared+' shared</small></button>'; }).join('')+'</div>';
    ...
    async function openVenue(input, cityHint){
      ...
      try{
        ...
        if(v.lat!=null && v.lng!=null) h+='<div class="sec-h" style="margin-top:26px">Location</div>'+(v.address?'<div class="venue-addr">'+esc(v.address)+'</div>':'')+'<div id="venue-loc" class="venue-map" style="height:300px"></div>';
        ...
        if(vpst && pst.length){
          vpst.innerHTML='<div class="sec-h" style="margin-top:26px">Past events</div><ul class="gig-history">'+pst.map(function(g){
            var d=fmtDateLabel(String(g.event_date).slice(0,10));
            var line=(g.lineup||[]).map(function(a){ return a.name; }).filter(Boolean).slice(0,3).join(', ');
            var label=g.title||line||'Event';
            return '<li class="gh-row" data-open-evt="'+esc(String(g.id))+'"><span class="gh-date">'+esc(d)+'</span>'+
    ...
    // dock prev/next walks the whole line-up; the name links to the artist's page. Wanna-see
    // / seen-live / view-artist also live in the ••• overflow (openBillMenu), fresh each open.
    function billRowsHTML(items){
      return '<div class="ev-bill">'+items.map(function(a, i){
        var av=a.image?('background-image:url('+imgUrl(a.image,80)+')'):'';
        ...
        var nameInner='<span class="bill-nline"><span class="bill-nm">'+esc(a.name)+'</span>'+badge+'</span>'+why;
        // name → the artist's page; only the photo (▶) plays. No-slug rows fall back to play.
        var nameEl=a.slug
          ? '<button class="bill-nm-wrap bill-nm-link" type="button" data-bill-go="'+esc(a.slug)+'" aria-label="View '+esc(a.name)+'">'+nameInner+'</button>'
          : '<button class="bill-nm-wrap" type="button" data-bill-i="'+i+'" aria-label="Play '+esc(a.name)+'">'+nameInner+'</button>';
        return '<div class="bill-row">'+
          '<span class="bill-av" data-bill-i="'+i+'" style="'+av+'"><span class="bav-play" aria-hidden="true">&#9654;</span></span>'+
          nameEl+
          '<button class="bill-more" type="button" data-bill-more="'+i+'" aria-label="More for '+esc(a.name)+'">'+DOTS_SVG+'</button>'+
        '</div>';
      }).join('')+'</div>';
    }
    function mountBill(items){
      var box=document.getElementById('ev-bill'); if(!box) return;
      if(!items.length){ box.innerHTML='<div class="sec-h" style="margin-top:26px">Line-up</div><p style="color:var(--muted);font:500 var(--fs-sm) var(--body)">To be announced.</p>'; return; }
      box.innerHTML='<div class="sec-h" style="margin-top:26px">The bill<span class="ct">'+items.length+'</span></div>'+billRowsHTML(items);
      var q=items.map(function(a){ return { slug:a.slug, name:a.name, poster:a.image }; });
      // photo ▶ → play that artist (jumps the queue to them; dock prev/next walks the rest)
      box.querySelectorAll('[data-bill-i]').forEach(function(el){ el.addEventListener('click',function(e){ e.stopPropagation(); playYtList(q, +el.dataset.billI); }); });
      // name → the artist's page
      box.querySelectorAll('[data-bill-go]').forEach(function(el){ el.addEventListener('click',function(e){ e.stopPropagation(); openArtist(el.dataset.billGo); }); });
      // ••• → wanna-see / seen-live / view-artist
      box.querySelectorAll('.bill-more[data-bill-more]').forEach(function(b){ b.addEventListener('click',function(e){ e.stopPropagation(); openBillMenu(items[+b.dataset.billMore], q, +b.dataset.billMore); }); });
      var pb=document.getElementById('ev-playbill'); if(pb){ pb.onclick=function(){ playYtList(q,0); }; }
    }
    function openBillMenu(a, q, i){
      if(!a) return;
      var st=artStatus(a.slug);
      openActionSheet({ title:a.name, items:[
        { label:'Play', svg:'<svg viewBox="0 0 24 24" fill="currentColor"><path d="M8 5v14l11-7z"/></svg>', run:function(){ playYtList(q, i); } },
        { label:'Wanna see', svg:HEART_SVG, on:st==='wanna_see', run:function(){ toggleArtist(a.slug,{name:a.name}); } },
        { label:'Seen live', svg:CHECK_SVG, on:st==='seen', run:function(){ toggleSeen(a.slug,{name:a.name}); } },
        { label:'View artist', svg:EXPAND_SVG, run:function(){ openArtist(a.slug); } }
      ]});
    }
    function eventPageHTML(ev){
      ...
      var evHeart = eid ? '<button class="act-btn act-2nd heart'+evPicked+'" type="button" data-pick-evt="'+esc(eid)+'" data-title="'+esc(ev.title||'')+'" data-venue="'+esc(ev.venue||'')+'" data-day="'+esc(ev._day||'')+'" aria-pressed="'+(evPicked?'true':'false')+'" aria-label="Save this event">'+HEART_SVG+'<span>Save</span></button>' : '';
      ...
      return '<div class="ev-layout">'+
          ...
          '<div class="ev-col-main">'+
            '<div class="event-head-tx">'+
              ...
              '<div class="sheet-meta"><b class="venue-link" data-venue="'+esc(ev.venue||'')+'">'+esc(ev.venue||'TBA')+'</b><span id="ev-when">'+whenStr+'</span>'+priceTag+'</div>'+
            '</div>'+
            ...
            '<div id="ev-meta-rich"><div class="sk" style="height:18px;width:55%;border-radius:6px;margin-top:16px"></div><div class="sk" style="height:13px;width:92%;border-radius:6px;margin-top:11px"></div><div class="sk" style="height:13px;width:78%;border-radius:6px;margin-top:7px"></div></div>'+   /* genre chips + blurb — skeleton reserves space; renderEventRich swaps in place */
    ...
    function renderEventRich(ev){
      ...
      if(top){
        ...
        if(ev.description) th+='<p class="seo-desc">'+esc(ev.description)+'</p>';
        top.innerHTML=th;
      ...
      if(rich){
        ...
        if(ev.venueLat!=null && ev.venueLng!=null){
          rh+='<div class="sec-h" style="margin-top:26px">Location</div>'+(ev.venueAddress?'<div class="venue-addr">'+esc(ev.venueAddress)+'</div>':'')+
            ...
            '<div id="ev-loc" class="venue-map" style="height:240px"></div>';
        }
    ...
    function mapEventDetail(d){
      return { id:String(d.id), title:d.title, venue:d.venue, venue_slug:d.venue_slug, poster:d.poster_url,
        ...
        city:d.city, genres:d.genres||[], description:d.description||'', logged:!!d.logged,
    ...
    var lgArtists=[], lgSel=null, lgEventList=[], _logHint=null;
    ...
    function logFormHTML(){
      var cityOpts=CITY_LIST.slice().sort(function(a,b){ return a.name.localeCompare(b.name); }).map(function(c){
        return '<option value="'+esc(c.slug)+'"'+(c.slug===currentCity?' selected':'')+'>'+esc(c.name)+'</option>'; }).join('');
      return '<h2 class="page-title">Log a gig</h2>'+
        ...
        '<div class="sub" style="margin-bottom:16px">Pick the night you went to &mdash; straight from our listings. It lands in your diary and marks the line-up as seen.</div>'+
        '<div style="max-width:540px">'+
          '<div id="lg-preselect" style="display:none"></div>'+
          '<div id="lg-pickers">'+
            '<div class="lg-field"><label>City</label><select id="lg-city" class="lg-select">'+cityOpts+'</select></div>'+
            '<div class="lg-field"><label>Venue</label><div class="sg-wrap"><input type="text" id="lg-venue-in" placeholder="Search venues…" autocomplete="off"><div class="sg-drop" id="lg-venue-drop"></div></div></div>'+
            '<div class="lg-field" id="lg-events-field" style="display:none"><label>Which night?</label><input type="date" id="lg-date" style="margin-bottom:10px"><div class="lg-events" id="lg-events"></div></div>'+
          '</div>'+
          '<div id="lg-extra" style="display:none">'+
            ...
            '<button id="lg-save" type="button" style="width:100%;background:var(--accent);color:var(--accent-ink);border:none;border-radius:10px;padding:14px;font:700 var(--fs-xs) var(--body);letter-spacing:0.14em;text-transform:uppercase;cursor:pointer">Save to diary</button>'+
            '<div class="err" id="lg-err" style="color:var(--accent);font-size:var(--fs-sm);min-height:1em;margin-top:8px"></div>'+
          '</div>'+
        '</div>';
    }
    // Render the venue's indexed nights as a selectable list.
    function renderLgEvents(){
      var el=document.getElementById('lg-events'); if(!el) return;
      if(!lgEventList.length){ var dt=document.getElementById('lg-date'); el.innerHTML='<div class="empty" style="padding:6px 0">'+((dt&&dt.value)?'No indexed night here on that date — try another date.':'No indexed nights at this venue yet — try another.')+'</div>'; return; }
    ...
    // Load the indexed nights for a chosen venue (past nights from /api/venue).
    // A set date filters to that exact day (any depth); no date → recent nights.
    function loadLgVenue(name){
      lgSel=null; lgEventList=[];
      var city=document.getElementById('lg-city').value;
      document.getElementById('lg-venue-in').value=name;
      var field=document.getElementById('lg-events-field'); field.style.display='';
      document.getElementById('lg-events').innerHTML='<div class="empty" style="padding:6px 0">Loading nights…</div>';
      document.getElementById('lg-extra').style.display='none';
      var dt=document.getElementById('lg-date'), on=(dt&&dt.value)?('&on='+encodeURIComponent(dt.value)):'';
      pubGet('/api/venue?name='+encodeURIComponent(name)+'&city='+encodeURIComponent(city)+on).then(function(v){
        lgEventList=(v&&v.past)||[]; renderLgEvents();
      }).catch(function(){ lgEventList=[]; renderLgEvents(); });
    }
    ...
    function openLog(prefill){
      setRoute('log');
      lgArtists=[]; lgSel=null; lgEventList=[];
      _logHint = (prefill&&prefill.seenName) ? prefill.seenName : null;
      showPage(logFormHTML());
      wireLog();
      if(prefill&&prefill.eventId) preselectLogEvent(prefill.eventId);
      // Coming from "log this night" — jump straight to the venue's nights.
      else if(prefill&&prefill.venue) loadLgVenue(prefill.venue);
    }
    function wireLog(){
      ...
      // Re-scope the venue search when the city changes; reset the night list + date.
      document.getElementById('lg-city').addEventListener('change',function(){
        lgSel=null; lgEventList=[];
        document.getElementById('lg-venue-in').value='';
        var dt=document.getElementById('lg-date'); if(dt) dt.value='';
        document.getElementById('lg-events-field').style.display='none';
        document.getElementById('lg-extra').style.display='none';
      });
      // Date filter — re-query the chosen venue's nights for that exact day.
      var dEl=document.getElementById('lg-date');
      if(dEl) dEl.addEventListener('change',function(){ var nm=document.getElementById('lg-venue-in').value.trim(); if(nm) loadLgVenue(nm); });
      // Pick an indexed night at the chosen venue.
      document.getElementById('lg-events').addEventListener('click',function(e){ var b=e.target.closest('[data-evi]'); if(b) selectLgEvent(lgEventList[+b.dataset.evi]); });
      // Venue picker — scoped to the selected city; shows top venues on focus.
      var vin=document.getElementById('lg-venue-in'), vdrop=document.getElementById('lg-venue-drop');
      function venueFetch(){ var city=document.getElementById('lg-city').value, qv=vin.value.trim();
        return pubGet('/api/venues?city='+encodeURIComponent(city)+(qv.length>=2?('&q='+encodeURIComponent(qv)):'')).then(function(j){ return j.venues||[]; }); }
      var curV=[];
      function showV(list){ curV=list; if(!list.length){ vdrop.classList.remove('show'); vdrop.innerHTML=''; return; }
        vdrop.innerHTML=list.map(function(v,i){ return '<button type="button" data-vi="'+i+'">'+esc(v.name)+(v.area?(' <small>'+esc(v.area)+'</small>'):'')+'</button>'; }).join('');
        vdrop.classList.add('show'); }
      var runV=debounce(function(){ venueFetch().then(showV); },200);
      vin.addEventListener('focus',function(){ if(!vin.value.trim()) venueFetch().then(showV); });
      vin.addEventListener('input',runV);
      vdrop.addEventListener('mousedown',function(e){ var b=e.target.closest('[data-vi]'); if(b){ e.preventDefault(); vdrop.classList.remove('show'); loadLgVenue(curV[+b.dataset.vi].name); } });
      vin.addEventListener('blur',function(){ setTimeout(function(){ vdrop.classList.remove('show'); },160); });
    }
    async function saveLog(){
      ...
      try{
        var r=await authedFetch('/api/diary',{ method:'POST', headers:{'Content-Type':'application/json'}, body:JSON.stringify({
          date:lgSel.date||null, venue:lgSel.venue||null,
    ...
    function cardHTML(ev){
      ...
      var dayLab = isTon ? 'Tonight' : (ev._day||'');
      var t1=time12(ev.startTime);
      // meta under the image, two rows: date · time  /  venue · city
      var mWhen = [dayLab, t1].filter(Boolean).join(' &middot; ');
      ...
      return '<article class="gcard evc"'+(eid?' data-evt="'+esc(eid)+'"':'')+'>'+
        '<div class="evc-hero">'+
          ...
          (eid?'<button class="heart evc-heart'+picked+'" type="button" data-pick-evt="'+esc(eid)+'" data-title="'+esc(ev.title||'')+'" data-venue="'+esc(ev.venue||'')+'" data-day="'+esc(ev._day||'')+'" aria-pressed="'+(picked?'true':'false')+'" aria-label="Save this event">'+HEART_SVG+'</button>':'')+
    ...
    function renderEvents(events, eyebrowPrefix){
      ...
      if(evs.length===0){ gridEl.innerHTML='<div class="status">Nothing on this day. <button class="more-btn" id="grid-tryweekend">Try the weekend &rarr;</button></div>'; var _tw=document.getElementById('grid-tryweekend'); if(_tw) _tw.addEventListener('click',function(){ loadWeekend(); }); _gridEvents=[]; _genreFilter=null; _priceFilter=null; _nearFilter=null; syncGenreChip(); syncPriceChip(); syncNearChip(); return; }
    ...
    function buildCal(){
      ...
      // mode toggle
      if(!calOnDay) h+='<div class="cal-modes"><button type="button" data-cmode="day" class="'+(calMode==='day'?'on':'')+'">Day</button><button type="button" data-cmode="month" class="'+(calMode==='month'?'on':'')+'">Month</button><button type="button" data-cmode="range" class="'+(calMode==='range'?'on':'')+'">Range</button></div>';
      // hint line
      if(calMode==='month'){
      ...
      for(var day=1;day<=days;day++){
        var d=new Date(y,m,day), di=isoOf(d), past=d<tdy, we=(d.getDay()===5||d.getDay()===6);
        var cls='cal-cell'+(past?' past':'')+(we?' we':'')+(di===isoOf(tdy)?' today':'');
      ...
      h+='</div><div class="cal-foot"><button id="cal-today" type="button">Today</button>'+applyBtn+'<button id="cal-close" type="button">Close</button></div>';
      calEl.innerHTML=h;
      // wire mode toggle
      ...
      document.getElementById('cal-today').onclick=function(){ closeCal(); if(calOnDay) calOnDay(todayISO()); else load(todayISO()); };
      ...
      if(applyBtn){
        document.getElementById('cal-apply').onclick=function(){
          ...
          closeCal(); loadRange(calRangeStart, calRangeEnd);
        };
      }
