    ...
    // scroll effect: event-card play buttons invert (salmon triangle / black circle) when they rise into the
    // top half of the viewport — a positional effect driven by IntersectionObserver (no per-frame scroll work).
    var _ppIO = ('IntersectionObserver' in window) ? new IntersectionObserver(function(ents){
      ents.forEach(function(en){ en.target.classList.toggle('inv', en.isIntersecting); });
    }, { rootMargin:'0px 0px -50% 0px', threshold:0 }) : null;
    function wireEvcCards(scope, evById){
      ...
      scope.querySelectorAll('.gcard.evc').forEach(function(card){
        ...
        var pb=card.querySelector('.evc-play'); if(pb){ pb.addEventListener('click',function(e){ e.stopPropagation(); playYtList(q,0); }); if(_ppIO) _ppIO.observe(pb); }
    ...
    function _renderStrip(coreHTML, lastISO){
      ...
      var html=coreHTML, last=lastISO, approx=stripEl.clientWidth ? Math.ceil(stripEl.clientWidth/56)+3 : 22;
      for(var k=0;k<approx;k++){ last=_addDaysISO(last,1); html+=_dtTile(last); }
