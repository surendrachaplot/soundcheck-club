    ...
    document.getElementById('bnav-map').addEventListener('click',function(){ openMap(); });
    ...
    function cardHTML(ev){
      ...
      var eid=ev.id!=null?String(ev.id):'';
      var canPlay = eid && (ev.artists||[]).length;   // play the line-up as a YouTube queue
      return '<div class="gcard"'+(eid?' data-evt="'+esc(eid)+'"':'')+'>'+
        '<div class="card-poster-wrap">'+
          ...
          (canPlay?'<button class="card-play" type="button" aria-label="Play the line-up"><svg viewBox="0 0 24 24" fill="currentColor"><path d="M8 5v14l11-7z"/></svg></button>':'')+
        '</div>'+
        pick+
    ...
    function paintCurrentGrid(){
      ...
      gridEl.querySelectorAll('.gcard').forEach(function(card,i){
        ...
        var rb=card.querySelector('.card-play'); if(rb) rb.addEventListener('click',function(e){ e.stopPropagation(); startEventRadio(ev); });
      });
      applyShared();
      decorateProof(list);
    }
    ...
    document.querySelectorAll('.nl[data-nav]').forEach(function(b){ b.addEventListener('click',function(){ var n=b.dataset.nav; if(n==='discover') openDiscover(); else if(n==='scenes') openScenes(); else if(n==='map') openMap(); else if(n==='upcoming') openUpcoming(); }); });
