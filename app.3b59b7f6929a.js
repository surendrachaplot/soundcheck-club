    ...
    var _stripMid=[], _stripExpanded=false;
    ...
    function buildDayStrip(){
      if(!stripEl) return;
      var today=todayISO(), fri=nextFridayISO();
      ...
      // keep collapsed unless the user expanded it, or the selected day is hidden in there
    ...
    buildDayStrip();
    // one delegated listener (the strip re-renders on expand, so per-tile handlers would go stale)
    ...
    function syncChips(){
      ...
      // a selected weekday hidden under "···" → rebuild so it's revealed + highlightable
      if(sel && _stripMid.indexOf(sel)>=0 && !stripEl.querySelector('.dt[data-iso="'+sel+'"]')) buildDayStrip();
      var hit=null;
