    ...
    function updatePP(playing){ dockPP.innerHTML = playing ? PP_PAUSE : PP_PLAY; dockPP.setAttribute('aria-label', playing?'Pause':'Play'); }
    ...
    function wireCard(scope){
      ...
      // Heart / Seen / RSVP toggles + per-item share (delegated)
    ...
    // The cassette brand mark — reused as the marquee separator and anywhere a small icon glyph is needed.
    var CASSETTE_SVG='<svg viewBox="0 0 32 32" fill="none" stroke="currentColor" stroke-width="2.2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><rect x="3" y="8" width="26" height="16" rx="3"/><circle cx="11.5" cy="16" r="2.7"/><circle cx="20.5" cy="16" r="2.7"/><circle cx="11.5" cy="16" r="0.7" fill="currentColor" stroke="none"/><circle cx="20.5" cy="16" r="0.7" fill="currentColor" stroke="none"/></svg>';
    ...
    function toggleSeen(slug, meta){ if(!slug) return; if(!currentUser){ openAuth(); return; } setArtist(slug, artStatus(slug)==='seen' ? null : 'seen', meta&&meta.name); }
    ...
    function renderAccount(){
      ...
      if(currentUser){
        ...
        menu.querySelectorAll('.am-i').forEach(function(b){ b.addEventListener('click',function(e){ e.stopPropagation(); menu.classList.remove('show'); var a=b.dataset.am;
          ...
          else if(a==='diary') openAccount('diary');
    ...
    function openAccount(initialTab){
      ...
      Promise.all([
        ...
        var p=res[0]||{}, up=res[1]||{};
        var s=(p&&p.shelves)||{wannaSee:[],seen:[],events:[],venues:[]}, diary=(p&&p.diary)||[];
        var followers=p.followers||0, following=p.following||0;
        ...
        var head='<div class="prof-head">'+
          '<span class="prof-av" style="background-image:'+avatarBg(handle,currentUser.avatar)+'"></span>'+
          '<div class="prof-id"><div class="prof-nm">u/'+esc(handle)+'</div><div class="prof-hn">your account</div></div>'+
          statLineHTML(s.seen.length, s.wannaSee.length, diary.length, followers)+
          '<div class="acc-corner"><button class="acc-gear" id="acc-gear" type="button" aria-label="Settings">'+GEAR+'</button></div>'+
          '</div>';
        // Coming-up banner — soonest show across the artists + venues you follow
        // (an event can appear in both feeds → dedupe by id before counting).
        var seen={}, feed=[].concat(up.artists||[], up.venues||[]).filter(function(e){ if(!e||!e.event_date||seen[e.id]) return false; seen[e.id]=1; return true; }).sort(function(a,b){ return String(a.event_date).localeCompare(String(b.event_date)); });
        var bInner;
        if(feed.length){
          var nx=feed[0], who=(nx.lineup&&nx.lineup[0]&&nx.lineup[0].name)||nx.title||'A show', when=fmtDateLabel(String(nx.event_date).slice(0,10)), more=feed.length-1;
          bInner='<div><div class="bk">Coming up</div><div class="bv">'+esc(who)+(nx.venue?' &middot; '+esc(nx.venue):'')+' &mdash; '+esc(when)+(more>0?'  +'+more+' more':'')+'</div></div>';
        } else {
          bInner='<div><div class="bk">Coming up</div><div class="bv">Next shows from the artists &amp; venues you follow</div></div>';
        }
        var banner='<button class="acc-banner" id="acc-coming" type="button">'+bInner+'<span class="bch">&rsaquo;</span></button>';
        // Share-your-taste-card CTA — same as the profile page (you have picks).
        var tasteCTA=((s.wannaSee&&s.wannaSee.length)||(s.seen&&s.seen.length)) ? '<button class="tc-cta" id="acc-tc" data-handle="'+esc(handle)+'" style="margin-top:16px">◆ Share your taste card</button>' : '';
        var tabs='<div class="prof-tabs"><button data-atab="shelves" class="on">Shelves</button><button data-atab="diary">Diary</button><button data-atab="network">Network</button></div><div id="acc-tabbody"></div>';
        var ab=document.getElementById('acc-body'); if(!ab) return; ab.innerHTML=head+banner+sceneStatsHTML(p.stats,true)+tasteCTA+tabs;
        document.getElementById('acc-coming').addEventListener('click',openUpcoming);
        var atc=document.getElementById('acc-tc'); if(atc) atc.addEventListener('click',function(){ openTasteCard(handle); });
        // settings gear → popover (handle / photo / feed / sign out)
        function closePop(){ var pp=document.getElementById('acc-pop'); if(pp) pp.remove(); document.removeEventListener('click',closePop); }
        document.getElementById('acc-gear').addEventListener('click',function(e){
          e.stopPropagation();
          if(document.getElementById('acc-pop')){ closePop(); return; }
          ...
          var pop=document.createElement('div'); pop.className='acc-pop'; pop.id='acc-pop';
          pop.innerHTML=
            '<button id="ap-handle">'+ic('<circle cx="12" cy="12" r="4"/><path d="M16 8v5a3 3 0 0 0 6 0v-1a10 10 0 1 0-3.92 7.94"/>')+'Change handle</button>'+
            '<button id="ap-photo">'+ic('<path d="M14.5 4h-5L7 7H4a2 2 0 0 0-2 2v9a2 2 0 0 0 2 2h16a2 2 0 0 0 2-2V9a2 2 0 0 0-2-2h-3z"/><circle cx="12" cy="13" r="3"/>')+(currentUser.avatar?'Change photo':'Add a photo')+'</button>'+
            (currentUser.avatar?'<button id="ap-photo-rm">'+ic('<path d="M3 6h18"/><path d="M19 6v14a2 2 0 0 1-2 2H7a2 2 0 0 1-2-2V6"/><path d="M8 6V4a2 2 0 0 1 2-2h4a2 2 0 0 1 2 2v2"/>')+'Remove photo</button>':'')+
            '<button id="ap-feed">'+ic('<path d="M4 11a9 9 0 0 1 9 9"/><path d="M4 4a16 16 0 0 1 16 16"/><circle cx="5" cy="19" r="1"/>')+'Feed</button>'+
            '<div class="sep"></div>'+
            '<button id="ap-out" class="danger">'+ic('<path d="M9 21H5a2 2 0 0 1-2-2V5a2 2 0 0 1 2-2h4"/><polyline points="16 17 21 12 16 7"/><line x1="21" x2="9" y1="12" y2="12"/>')+'Sign out</button>';
          document.querySelector('.acc-corner').appendChild(pop);
          pop.querySelector('#ap-handle').onclick=function(){ closePop(); openHandleModal(true); };
          pop.querySelector('#ap-photo').onclick=function(){ closePop(); pickAvatar(); };
          var rmp=pop.querySelector('#ap-photo-rm'); if(rmp) rmp.onclick=function(){ closePop(); uploadAvatar(null); };
          pop.querySelector('#ap-feed').onclick=function(){ closePop(); openFeed(); };
          pop.querySelector('#ap-out').onclick=function(){ closePop(); signOut(); };
          setTimeout(function(){ document.addEventListener('click',closePop); },0);
        });
        var body=document.getElementById('acc-tabbody');
        function showTab(t){
          if(t==='shelves'){ body.innerHTML=shelfSecHTML(s, handle); wireShelfBody(body, s, handle); }
          else if(t==='diary'){ body.innerHTML='<div class="sec-h">Diary <span class="more" id="acc-log">+ Log a gig</span></div>'+diaryTLHTML(diary, true); var lg=document.getElementById('acc-log'); if(lg) lg.addEventListener('click',openLog); wireDiary(body, true, function(){ openAccount('diary'); }); }
          else { body.innerHTML='<div class="panel-stat"><span><b>'+followers+'</b> followers</span><span><b>'+following+'</b> following</span></div>'; }
        }
        var startTab=(initialTab==='diary'||initialTab==='network')?initialTab:'shelves';
        showTab(startTab);
        ab.querySelectorAll('.prof-tabs [data-atab]').forEach(function(x){ x.classList.toggle('on', x.dataset.atab===startTab); });
        ab.querySelectorAll('.prof-tabs [data-atab]').forEach(function(b){ b.addEventListener('click',function(){ b.parentNode.querySelectorAll('button').forEach(function(x){x.classList.remove('on');}); b.classList.add('on'); showTab(b.dataset.atab); }); });
    ...
    function showPage(html){
      pageView.innerHTML='<div class="page-head"><button class="page-back" id="page-back" type="button">&lsaquo; Back</button></div><div id="page-body">'+html+'</div>';
    ...
    function hidePage(){
      ...
      document.body.classList.remove('page-on'); pageView.hidden=true; pageView.innerHTML='';
      resetSeo();
    ...
    function ptile(slug,name,seen,image){
      ...
      return '<button class="ptile" data-artist="'+esc(slug)+'"><span class="ptile-img'+(image?' has-img':'')+'" style="background:'+artGrad(name||slug)+'">'+img+(seen?'<span class="sd">&#10003;</span>':'')+'</span><span class="nm">'+esc(name||slug)+'</span></button>';
    }
    ...
    function sceneStatsHTML(st, mine){
      if(!st || !st.gigs) return '';
      var tag=function(label,n){ return '<span class="sc-tag">'+esc(label)+(n?' <small>'+n+'</small>':'')+'</span>'; };
      var rows='';
      if((st.topGenres||[]).length) rows+='<div class="sc-srow"><span class="sc-sk">Sound</span><div>'+st.topGenres.map(function(g){ return tag(g); }).join('')+'</div></div>';
      if((st.topVenues||[]).length) rows+='<div class="sc-srow"><span class="sc-sk">Venues</span><div>'+st.topVenues.map(function(v){ return tag(v.venue,v.n); }).join('')+'</div></div>';
      if((st.topArtists||[]).length) rows+='<div class="sc-srow"><span class="sc-sk">Most seen</span><div>'+st.topArtists.map(function(a){ return tag(a.name,a.n); }).join('')+'</div></div>';
      return '<div class="scene-stats"><div class="sec-h">'+(mine?'Your scene':'Scene')+'</div><div class="sc-nums"><span><b>'+st.gigs+'</b>gigs logged</span>'+(st.thisYear?'<span><b>'+st.thisYear+'</b>this year</span>':'')+'</div>'+rows+'</div>';
    }
    ...
    function shelfSecHTML(s, handle){
      ...
      var x='<div class="sec-h">Wanna see<span class="ct">'+s.wannaSee.length+'</span>'+(s.wannaSee.length?radBtn('wanna'):'')+'</div>'+(s.wannaSee.length?'<div class="ptiles ptiles-wide">'+s.wannaSee.map(function(a){return ptile(a.slug,a.name,false,a.image);}).join('')+'</div>':'<div class="empty" style="padding:0 0 4px">Nothing yet.</div>');
      x+='<div class="sec-h" style="margin-top:26px">Seen live<span class="ct">'+s.seen.length+'</span>'+(s.seen.length?radBtn('seen'):'')+'</div>'+(s.seen.length?'<div class="ptiles ptiles-wide">'+s.seen.map(function(a){return ptile(a.slug,a.name,true,a.image);}).join('')+'</div>':'<div class="empty" style="padding:0">Nothing yet.</div>');
    ...
    async function openProfile(handle){
      ...
      try{
        ...
        var p=await r.json();
        var s=p.shelves||{wannaSee:[],seen:[],events:[]}, diary=p.diary||[];
        var head='<div class="prof-head"><span class="prof-av" style="background-image:'+avatarBg(p.handle,p.avatar)+'"></span>'+
          '<div class="prof-id"><div class="prof-nm">'+esc(p.displayName||('u/'+p.handle))+'</div><div class="prof-hn">u/'+esc(p.handle)+'</div></div>'+
          statLineHTML(s.seen.length, s.wannaSee.length, diary.length, p.followers||0)+
          (p.isMe?'':'<button class="follow-btn'+(p.isFollowing?' following':'')+'" id="follow-btn" data-handle="'+esc(p.handle)+'">'+(p.isFollowing?'Following':'Follow')+'</button>')+
          '</div>';
        if(!p.locked && ((s.wannaSee&&s.wannaSee.length)||(s.seen&&s.seen.length))) head += '<button class="tc-cta" id="prof-tc" data-handle="'+esc(p.handle)+'" style="margin-top:16px">◆ '+(p.isMe?'Share your taste card':(esc('u/'+p.handle)+'’s taste card'))+'</button>';
        if(p.locked){ openPanel(head+'<div class="empty" style="margin-top:18px">This profile is private.</div>'); wireProfile(); return; }
        openPanel(head+sceneStatsHTML(p.stats,p.isMe)+'<div class="prof-tabs"><button data-tab="shelves" class="on">Shelves</button><button data-tab="diary">Diary</button><button data-tab="network">Network</button></div><div id="prof-tabbody"></div>');
        wireProfile();
        var ptc=document.getElementById('prof-tc'); if(ptc) ptc.addEventListener('click',function(){ openTasteCard(ptc.dataset.handle); });
        var TAB={
          shelves:function(){ return shelfSecHTML(s, p.handle); },
          diary:function(){ return diaryTLHTML(diary, p.isMe); },
          network:function(){ return '<div class="panel-stat"><span><b>'+(p.followers||0)+'</b> followers</span><span><b>'+(p.following||0)+'</b> following</span></div>'; }
        };
        var body=document.getElementById('prof-tabbody');
        function showTab(t){ body.innerHTML=TAB[t](); if(t==='diary') wireDiary(body, p.isMe, function(){ openProfile(p.handle); }); else wireShelfBody(body, s, p.handle); }
        showTab('shelves');
        panelBody.querySelectorAll('.prof-tabs [data-tab]').forEach(function(b){ b.addEventListener('click',function(){ panelBody.querySelectorAll('.prof-tabs button').forEach(function(x){x.classList.remove('on');}); b.classList.add('on'); showTab(b.dataset.tab); }); });
    ...
    function mountEventCards(el, rows){
      ...
      el.querySelectorAll('.gcard').forEach(function(c){
        var ev=rmap[c.getAttribute('data-evt')]; if(!ev) return;
        var open=function(e){ e.preventDefault(); e.stopPropagation(); openSheet(ev); };
        var p=c.querySelector('.card-poster'); if(p) p.addEventListener('click',open);
        var t=c.querySelector('.title-btn'); if(t) t.addEventListener('click',open);
        var rb=c.querySelector('.ev-radio'); if(rb) rb.addEventListener('click',function(e){ e.stopPropagation(); startEventRadio(ev); });
      });
    }
    ...
    function openSearch(){
      ...
      var lastQ='', showing='';
      // recent searches sit as plain pills above the search box (no sub-heading)
    ...
    function mountVenueMap(el, city, opts){
      ...
      loadLeaflet(async function(){
        ...
        vs.forEach(function(v){
          ...
          var halo=L.circleMarker([v.lat,v.lng],{ radius:16, weight:0, fillColor:'#ff7a6b', fillOpacity:0.13 });
          var dot=L.circleMarker([v.lat,v.lng],{ radius:7, color:'#fff', weight:1.5, fillColor:'#ff7a6b', fillOpacity:1 });
    ...
    async function openArtist(slug){
      ...
      try{
        ...
        if(up && rows.length){
          ...
          up.querySelectorAll('.gcard').forEach(function(c){
            var ev=rmap[c.getAttribute('data-evt')]; if(!ev) return;
            var open=function(e){ e.preventDefault(); e.stopPropagation(); openSheet(ev); };
            var p=c.querySelector('.card-poster'); if(p) p.addEventListener('click',open);
            var t=c.querySelector('.title-btn'); if(t) t.addEventListener('click',open);
    ...
    function toggleVenueFollow(v, btn){
      ...
      btn.textContent=willFollow?'Following':'Follow venue';
      ...
      authedFetch('/api/venue/follow',{ method:'POST', headers:{'Content-Type':'application/json'}, body:JSON.stringify({ slug:v.slug, name:v.name, follow:willFollow }) })
        .then(function(r){ if(!r.ok) throw 0; })
        .catch(function(){ btn.classList.toggle('following', !willFollow); btn.textContent=!willFollow?'Following':'Follow venue'; v.following=!willFollow; showToast('Couldn’t update.'); });
    }
    ...
    async function openVenue(input, cityHint){
      ...
      try{
        ...
        if(v.slug){ h+='<div class="artist-actions" style="max-width:540px"><button class="follow-btn'+(v.following?' following':'')+'" id="vf-btn" type="button">'+(v.following?'Following':'Follow venue')+'</button>'+
          '<button class="ev-radio been-btn'+(v.myReview?' on':'')+'" id="vp-been" type="button">'+CHECK_SVG+'<span>'+(v.myReview?'You&rsquo;ve been':'I&rsquo;ve been')+'</span></button>'+
          (vNext?'<button class="ev-radio" id="vp-radio" type="button">'+RADIO_SVG+'<span>Play next night</span></button>':'')+'</div>'; }
    ...
    function billRowsHTML(items){
      return '<div class="ev-bill">'+items.map(function(a){
        ...
        // per-artist wanna-see + seen-live toggles — same components as the grid/artist page,
        // so refreshArtistButtons() keeps them in sync everywhere via data-pick-art/data-seen-art.
        var heart='<button class="heart'+(st==='wanna_see'?' on':'')+'" type="button" data-pick-art="'+esc(a.slug)+'" data-name="'+esc(a.name)+'" aria-pressed="'+(st==='wanna_see'?'true':'false')+'" aria-label="Wanna see '+esc(a.name)+'">'+HEART_SVG+'</button>';
        var seen='<button class="seen-btn'+(st==='seen'?' on':'')+'" type="button" data-seen-art="'+esc(a.slug)+'" data-name="'+esc(a.name)+'" aria-label="Seen '+esc(a.name)+' live">'+CHECK_SVG+'</button>';
        return '<div class="bill-row"><button class="bill-play" type="button" data-slug="'+esc(a.slug)+'" data-name="'+esc(a.name)+'" aria-label="Play '+esc(a.name)+'">&#9654;</button>'+
          '<span class="bill-av" data-artist="'+esc(a.slug)+'" style="'+av+'"></span>'+
          '<button class="bill-nm-wrap" type="button" data-artist="'+esc(a.slug)+'"><span class="bill-nline"><span class="bill-nm">'+esc(a.name)+'</span>'+badge+'</span>'+why+'</button>'+
          '<div class="bill-acts">'+heart+seen+'</div></div>';
      }).join('')+'</div>';
    }
    function mountBill(items){
      ...
      box.querySelectorAll('.bill-play[data-slug]').forEach(function(b,i){ b.addEventListener('click',function(e){ e.stopPropagation(); playYtList(q, i); }); });
      box.querySelectorAll('[data-artist]').forEach(function(el){ el.addEventListener('click',function(){ openArtist(el.dataset.artist); }); });
      box.querySelectorAll('.heart[data-pick-art]').forEach(function(b){ b.addEventListener('click',function(e){ e.stopPropagation(); toggleArtist(b.dataset.pickArt,{name:b.dataset.name}); }); });
      box.querySelectorAll('.seen-btn[data-seen-art]').forEach(function(b){ b.addEventListener('click',function(e){ e.stopPropagation(); toggleSeen(b.dataset.seenArt,{name:b.dataset.name}); }); });
    ...
    function eventPageHTML(ev){
      ...
      var evHeart = eid ? '<button class="heart'+evPicked+'" type="button" data-pick-evt="'+esc(eid)+'" data-title="'+esc(ev.title||'')+'" data-venue="'+esc(ev.venue||'')+'" data-day="'+esc(ev._day||'')+'" aria-pressed="'+(evPicked?'true':'false')+'" aria-label="Save this event">'+HEART_SVG+'</button>' : '';
      var evShare = eid ? '<button class="share-btn" type="button" data-share-evt="'+esc(eid)+'" data-title="'+esc(ev.title||'')+'" data-venue="'+esc(ev.venue||'')+'" aria-label="Share this event">'+SHARE_SVG+'</button>' : '';
      ...
      var evLog = isPast ? '<button class="act-log'+(ev.logged?' on':'')+'" type="button" data-log-evt="'+esc(eid)+'">'+(ev.logged?'&#10003; Logged':'+ Add to log')+'</button>' : '';
      ...
      return '<div class="ev-layout">'+
          ...
          '<div class="ev-col-main">'+
            '<div class="event-head-tx">'+
              ...
              (eid ? '<div class="sheet-actions">'+(isPast ? (evLog+evShare+'<span class="lbl">Log it or share</span>') : (evHeart+evShare+'<span class="lbl">Save or share</span>'))+'</div>' : '')+
            '</div>'+
            ...
            ((ev.artists||[]).length?'<button class="ev-radio" id="ev-playbill" type="button" style="margin-top:22px">'+RADIO_SVG+'<span>Play the line-up</span></button>':'')+
    ...
    async function openEvent(id){
      ...
      if(cached){ openPanel(eventPageHTML(cached)); wireCard(document.getElementById('page-body')); mountBill(billItemsFromEv(cached)); }
      ...
      try{
        ...
        if(!cached){ openPanel(eventPageHTML(ev)); wireCard(document.getElementById('page-body')); mountBill(billItemsFromEv(ev)); }
    ...
    var lgArtists=[], lgSel=null, lgEventList=[];
    ...
    function logFormHTML(){
      ...
      return '<h2 class="page-title">Log a gig</h2><div class="sub" style="margin-bottom:16px">Pick the night you went to &mdash; straight from our listings. It lands in your diary and marks the line-up as seen.</div>'+
    ...
    async function saveLog(){
      ...
      try{
        ...
        // land on the diary (account → Diary tab); replace #log so Back doesn't
        // return to a now-stale form.
        history.replaceState({sc:1},'','#account'); openAccount('diary');
    ...
    function lineupHTML(ev, cap, inSheet){
      ...
      arts.forEach(function(a,idx){
        ...
        var heart=slug?'<button class="heart'+picked+'" type="button" data-pick-art="'+esc(slug)+'" data-name="'+esc(a.name)+'" aria-pressed="'+(picked?'true':'false')+'" aria-label="Wanna see '+esc(a.name)+'">'+HEART_SVG+'</button>':'';
        ...
        var anCls = (inSheet && slug) ? 'an linky' : 'an';
        var anAttr = (inSheet && slug) ? ' data-artist="'+esc(slug)+'" data-name="'+esc(a.name)+'"' : '';
    ...
    function cardHTML(ev){
      var time=formatTimeRange(ev.startTime, ev.endTime, ev._time);
      var pick=isPickVenue(ev.venue)?'<span class="pick">Pick</span>':'';
      var eid=ev.id!=null?String(ev.id):'';
      var evPicked = eid && isEventPicked(eid) ? ' on' : '';
      var actions = eid ? '<div class="card-actions">'+
        '<button class="heart'+evPicked+'" type="button" data-pick-evt="'+esc(eid)+'" data-title="'+esc(ev.title||'')+'" data-venue="'+esc(ev.venue||'')+'" data-day="'+esc(ev._day||'')+'" aria-pressed="'+(evPicked?'true':'false')+'" aria-label="Pick this event">'+HEART_SVG+'</button>'+
        '<button class="share-btn" type="button" data-share-evt="'+esc(eid)+'" data-title="'+esc(ev.title||'')+'" data-venue="'+esc(ev.venue||'')+'" aria-label="Share this event">'+SHARE_SVG+'</button>'+
      '</div>' : '';
      return '<div class="gcard"'+(eid?' data-evt="'+esc(eid)+'"':'')+'>'+
        '<button class="card-poster" type="button" aria-label="View '+esc(ev.title)+' lineup">'+posterHTML(ev)+'<span class="expand-badge">'+EXPAND_SVG+'</span></button>'+
        pick+
        '<h3><button class="title-btn" type="button">'+esc(ev.title)+'</button></h3>'+
        '<div class="meta"><b>'+esc(ev.venue||'TBA')+'</b>'+(ev._city?' &middot; '+esc(ev._city):'')+(ev._day?' &middot; '+esc(ev._day):'')+(time?' &middot; '+time:'')+'</div>'+
        actions+
        (eid?'<div class="proof" id="proof-'+esc(eid)+'"></div>':'')+
        ((eid&&(ev.artists||[]).length)?'<button class="ev-radio" type="button">'+RADIO_SVG+'<span>Lineup Radio</span></button>':'')+
        '<ul class="line">'+lineupHTML(ev,5,false)+'</ul>'+
        soloCtaHTML(ev)+
      '</div>';
    }
    ...
    var gridEl=document.getElementById('grid'), trackEl=document.getElementById('track');
    ...
    function paintCurrentGrid(){
      ...
      if(list.length){ gridEl.innerHTML=list.map(function(e){ return cardHTML(e); }).join(''); }
        ...
        var what = _genreFilter ? (esc(_genreFilter)+' events') : (_priceFilter ? 'events at this price' : 'events');
      ...
      var _cf=document.getElementById('grid-clearfilter'); if(_cf) _cf.addEventListener('click',function(){ _genreFilter=null; _priceFilter=null; if(typeof syncGenreChip==='function') syncGenreChip(); if(typeof syncPriceChip==='function') syncPriceChip(); paintCurrentGrid(); });
      wireCard(gridEl);
      gridEl.querySelectorAll('.gcard').forEach(function(card,i){
        var ev=list[i];
        function open(){ openSheet(ev); }
        var p=card.querySelector('.card-poster'); if(p) p.addEventListener('click',open);
        var t=card.querySelector('.title-btn'); if(t) t.addEventListener('click',open);
        var rb=card.querySelector('.ev-radio'); if(rb) rb.addEventListener('click',function(e){ e.stopPropagation(); startEventRadio(ev); });
      });
      applyShared();
      decorateProof(list);
    }
    ...
    function renderEvents(events, eyebrowPrefix){
      ...
      document.getElementById('eyebrow').textContent=eyebrowPrefix+' — '+evs.length+' event'+(evs.length===1?'':'s');
      if(evs.length===0){ gridEl.innerHTML='<div class="status">Nothing on this day. <button class="more-btn" id="grid-tryweekend">Try the weekend &rarr;</button></div>'; var _tw=document.getElementById('grid-tryweekend'); if(_tw) _tw.addEventListener('click',function(){ loadWeekend(); }); trackEl.innerHTML=''; _gridEvents=[]; _genreFilter=null; _priceFilter=null; syncGenreChip(); syncPriceChip(); return; }
      var ordered=orderEvents(evs);
      // ambient marquee: "<up to 5 artists> at <venue>" (duplicated for a seamless loop)
      function marqText(e){
        var names=(e.artists||[]).map(function(a){ return a.name; }).filter(Boolean).slice(0,5);
        if(!names.length) return e.title;            // fall back to the event title when no lineup
        return names.join(', ')+(e.venue?(' at '+e.venue):'');
      }
      var marq='', SEP='<span class="star">'+CASSETTE_SVG+'</span>';   // cassette icon as the marquee separator
      for(var k=0;k<2;k++){ ordered.forEach(function(e){ marq+='<span>'+esc(marqText(e))+'</span>'+SEP; }); }
      trackEl.innerHTML=marq;
      _gridEvents=ordered; _genreFilter=null; _priceFilter=null; closeGenreMenu(); closePriceMenu(); syncGenreChip(); syncPriceChip();
      paintCurrentGrid();
    ...
    var friBtn=document.getElementById('pick-fri'),
        satBtn=document.getElementById('pick-sat'),
        pickDateBtn=document.getElementById('pick-date');

    function syncChips(){
      friBtn.classList.toggle('on', activePick==='fri');
      satBtn.classList.toggle('on', activePick==='sat');
      var onDate = activePick==='date';
      pickDateBtn.classList.toggle('on', onDate);
      var lbl='Pick a date';
      if(onDate){
        if(mode==='range' && currentRange) lbl=fmtRange(currentRange[0], currentRange[1]);
        else if(currentDate) lbl=fmtDateLabel(currentDate);
      }
      document.getElementById('pick-date-label').textContent=lbl;
    }
    ...
    function setCity(slug){
      ...
      if(onScPath()){
        history.replaceState(history.state,'', cityPath(slug)+u.search+u.hash);
      } else {
    ...
    function openCityPicker(){
      ...
      ov.addEventListener('click',function(e){
        ...
        if(b){ close(); setCity(b.dataset.city); }
      });
    }
    ...
    friBtn.addEventListener('click',function(){ load(weekendDatesFor(0)[0], 'fri'); });
    satBtn.addEventListener('click',function(){ load(weekendDatesFor(0)[1], 'sat'); });
    // On Saturday the upcoming "Friday" pill points at yesterday — hide it so the
    // bar shows just Saturday (today) + Pick a date. Sun–Fri keep both pills.
    if(new Date().getDay()===6) friBtn.style.display='none';
    pickDateBtn.addEventListener('click',function(){ openCal(); });
