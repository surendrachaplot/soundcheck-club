    ...
    function openAccount(initialTab){
      ...
      Promise.all([
        ...
        ab.innerHTML=profileDashHTML(p);
        wireProfileDash(p);
        ...
        armSticky('u/'+p.handle, accPlay, ab.querySelector('.pd-cover')||ab);
    ...
    async function openProfile(handle){
      ...
      try{
        ...
        var p=await r.json();
        var s=p.shelves||{wannaSee:[],seen:[],events:[],venues:[]}, diary=p.diary||[];
        if(p.locked){ openPanel('<div class="pd-page">'+pdCoverHTML(p,s,diary)+'<div class="empty" style="margin-top:18px">This profile is private.</div></div>'); wireProfile(); armSticky(p.displayName||('u/'+p.handle), null, (panelBody&&panelBody.querySelector('.pd-cover'))||document.getElementById('page-body')); return; }
        openPanel(profileDashHTML(p));
        wireProfileDash(p);
        ...
        armSticky(p.displayName||('u/'+p.handle), profPlay, (panelBody&&panelBody.querySelector('.pd-cover'))||document.getElementById('page-body'));
