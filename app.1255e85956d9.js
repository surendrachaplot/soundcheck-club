    ...
    function pbProfileHTML(p){
      ...
      var acts = isMe
        ? '<button class="pf-btn primary" id="pf-log" type="button"><i class="ti ti-plus" aria-hidden="true"></i> Log a gig</button><button class="pf-btn" id="pb-share" type="button"><i class="ti ti-share-2" aria-hidden="true"></i> Share</button><button class="pf-btn" id="pb-edit" type="button"><i class="ti ti-settings" aria-hidden="true"></i> Edit</button>'
    ...
    function wirePbProfile(p){
      ...
      var lg=document.getElementById('pf-log'); if(lg) lg.addEventListener('click',function(){ openLog(); });
      // Coming up: each row opens its event; "See all" → the full Coming up page
      ...
      return '<h2 class="page-title">Add an event</h2>'+
        ...
        '<div style="max-width:540px">'+
          '<div class="lg-field"><label>Event link <small style="text-transform:none;letter-spacing:0;color:var(--muted)">(optional)</small></label>'+
            '<div style="display:flex;gap:8px"><input type="url" id="im-url" placeholder="https://…" autocomplete="off" style="flex:1"><button id="im-fetch" type="button" style="flex:0 0 auto;background:var(--surface-2);color:var(--fg);border:1px solid var(--border);border-radius:10px;padding:0 16px;font:700 var(--fs-xs) var(--body);text-transform:uppercase;letter-spacing:0.08em;cursor:pointer">Fetch</button></div>'+
          ...
          '<button id="im-save" type="button" style="width:100%;background:var(--accent);color:var(--accent-ink);border:none;border-radius:10px;padding:14px;font:700 var(--fs-xs) var(--body);letter-spacing:0.14em;text-transform:uppercase;cursor:pointer">Add event</button>'+
