    ...
    function openAccount(initialTab){
      ...
      Promise.all([
        ...
        var ab=document.getElementById('acc-body'); if(!ab) return; ab.innerHTML=head+banner+sceneStatsHTML(p.stats,true,s.seen)+tasteCTA+tabs; wireScene(ab);
        // entity-page sticky bar: your handle + back + a play button that starts your seen artists' radio
        var accPlay = (s.seen && s.seen.length) ? function(){ startListRadio(s.seen, 'shelf:'+handle+':seen'); } : null;
        armSticky('u/'+handle, accPlay, ab.querySelector('.prof-head')||ab);
    ...
    function sceneStatsHTML(st, mine, seen){
      ...
      var soundsHTML = genres.map(function(g,i){ return '<span class="scn-snd'+(i===0?' scn-lead':'')+'">'+(i===0?WAV:'')+esc(g)+(i===0?' <i>lead</i>':'')+'</span>'; }).join('');
      var venuesHTML = venues.map(function(v,i){
        var pct=Math.max(8, Math.round((v.n/maxN)*100));
        return '<div class="scn-vrow'+(i===0?' scn-top':'')+'"><span class="scn-badge">&times;'+v.n+'</span>'+
          '<div class="scn-vmid"><div class="scn-vname">'+BLD+esc(v.venue)+'</div><div class="scn-meter"><span style="width:'+pct+'%"></span></div></div>'+
          '<div class="scn-vct"><b>'+v.n+'</b> nights</div></div>';
      }).join('');
      var SHOW=8;
      var bubbles = seenA.slice(0,SHOW).map(function(a){
        ...
        return '<span class="scn-bub" data-artist="'+esc(a.slug||'')+'" style="'+bg+'" title="'+esc(a.name||'')+'"></span>';
      }).join('') + (seenA.length>SHOW ? '<span class="scn-bub scn-bub-more">+'+(seenA.length-SHOW)+'</span>' : '');
      var homeClub = venues[0] ? venues[0].venue : '';
      var strip = (homeClub?'Home club <b>'+esc(homeClub)+'</b>':'') + (homeClub&&sinceYear?' <i class="scn-dot"></i> ':'') + (sinceYear?'since <b>'+sinceYear+'</b>':'') + '<span class="scn-sn">'+esc(serial)+'</span>';
      return '<div class="scn-scene">'+
        '<div class="scn-title">'+TKT+(mine?'Your scene':'Scene')+'</div>'+
        '<div class="scn-stub"><div class="scn-stub-top"><div class="scn-admit">'+TKT+'Admit one</div><div class="scn-seal">'+CHK+'Verified '+yearTag+'</div></div>'+
          '<div class="scn-big"><b>'+st.gigs+'</b><span>nights logged</span></div>'+
          (st.thisYear?'<div class="scn-yr">'+st.thisYear+' this year</div>':'')+
          '<div class="scn-barcode"></div></div>'+
        (genres.length?'<div class="scn-block"><div class="scn-lbl">'+WAV+'Sound passport</div><div class="scn-sounds">'+soundsHTML+'</div></div>':'')+
        (venues.length?'<div class="scn-block"><div class="scn-lbl">'+BLD+'Venue stamps</div><div class="scn-venues">'+venuesHTML+'</div></div>':'')+
        (seenA.length?'<div class="scn-block scn-actsblock"><div class="scn-lbl">'+USR+'Acts seen <span class="scn-actsn">'+seenA.length+'<small>artists</small></span></div><div class="scn-bubbles">'+bubbles+'</div></div>':'')+
        '<div class="scn-strip">'+strip+'</div>'+
        '<div class="scn-foot"><span>'+TKT+'Soundcheck passport</span><span>'+(sinceYear?'Building since '+sinceYear:'')+'</span></div>'+
      '</div>';
    }
    // wire the artist bubbles in a freshly-mounted "Your scene" panel → artist pages
    function wireScene(scope){ (scope||document).querySelectorAll('.scn-bub[data-artist]').forEach(function(b){ if(b.dataset.artist) b.addEventListener('click',function(){ openArtist(b.dataset.artist); }); }); }
    ...
    async function openProfile(handle){
      ...
      try{
        ...
        if(p.locked){ openPanel(head+'<div class="empty" style="margin-top:18px">This profile is private.</div>'); wireProfile(); armSticky(p.displayName||('u/'+p.handle), null, (panelBody&&panelBody.querySelector('.prof-head'))||document.getElementById('page-body')); return; }
        openPanel(head+sceneStatsHTML(p.stats,p.isMe,s.seen)+'<div class="prof-tabs"><button data-tab="shelves" class="on">Shelves</button><button data-tab="diary">Diary</button><button data-tab="network">Network</button></div><div id="prof-tabbody"></div>');
        wireProfile(); wireScene(panelBody);
        ...
        armSticky(p.displayName||('u/'+p.handle), profPlay, (panelBody&&panelBody.querySelector('.prof-head'))||document.getElementById('page-body'));
