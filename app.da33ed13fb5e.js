    ...
    // ── day strip: a swipeable run of upcoming days + a calendar tile, replacing
    // the old Fri / Sat / Pick-a-date chips. Built once (today is fixed for the
    // session); syncChips() just re-highlights the selected day. ──
    var stripEl=document.getElementById('daystrip');
    function buildDayStrip(){
      if(!stripEl) return;
      var today=new Date(); today.setHours(12,0,0,0); var html='';
      for(var i=0;i<14;i++){
        var d=new Date(today); d.setDate(today.getDate()+i); var iso=isoOf(d);
        html += i===0
          ? '<button class="dt lab" type="button" data-iso="'+iso+'">Tonight</button>'
          : '<button class="dt" type="button" data-iso="'+iso+'"><span class="dt-w">'+esc(d.toLocaleDateString('en-GB',{weekday:'short'}))+'</span><span class="dt-n">'+d.getDate()+'</span></button>';
      }
      html += '<button class="dt dscal" id="dt-cal" type="button" aria-label="Pick any date"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M8 2v4"/><path d="M16 2v4"/><rect width="18" height="18" x="3" y="4" rx="2"/><path d="M3 10h18"/></svg></button>';
      stripEl.innerHTML=html;
      stripEl.querySelectorAll('.dt[data-iso]').forEach(function(b){ b.addEventListener('click',function(){ load(b.dataset.iso); }); });
      var c=document.getElementById('dt-cal'); if(c) c.addEventListener('click',function(){ openCal(); });
    }
    buildDayStrip();
    function syncChips(){
      if(!stripEl) return;
      var sel = (mode==='day' && currentDate) ? currentDate : null, hit=null;
      ...
      // only scroll when the selected day is off-screen — keep "Tonight" in view otherwise
      if(hit){ var l=hit.offsetLeft, r=l+hit.offsetWidth; if(l<stripEl.scrollLeft) stripEl.scrollLeft=Math.max(0,l-8); else if(r>stripEl.scrollLeft+stripEl.clientWidth) stripEl.scrollLeft=r-stripEl.clientWidth+8; }
    }
