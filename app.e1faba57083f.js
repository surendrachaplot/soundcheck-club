    ...
    // The bill = a YouTube queue. Tapping a row (photo or name) plays that artist and
    // sets the queue so the dock prev/next walks the whole line-up. The play ▶ button
    // is gone (the photo carries a play badge); wanna-see / seen-live / view-artist move
    // into the ••• overflow (openBillMenu), reading fresh status each open.
    function billRowsHTML(items){
      return '<div class="ev-bill">'+items.map(function(a, i){
        ...
        return '<div class="bill-row">'+
          ...
          '<button class="bill-nm-wrap" type="button" data-bill-i="'+i+'" aria-label="Play '+esc(a.name)+'"><span class="bill-nline"><span class="bill-nm">'+esc(a.name)+'</span>'+badge+'</span>'+why+'</button>'+
    ...
    function mountBill(items){
      ...
      // photo or name → play that artist (jumps the queue to them; dock prev/next walks the rest)
    ...
    function openBillMenu(a, q, i){
      ...
      openActionSheet({ title:a.name, items:[
        { label:'Play', svg:RADIO_SVG, run:function(){ playYtList(q, i); } },
    ...
    function eventPageHTML(ev){
      ...
      var evShare = eid ? '<button class="act-btn act-2nd share-btn" type="button" data-share-evt="'+esc(eid)+'" data-title="'+esc(ev.title||'')+'" data-venue="'+esc(ev.venue||'')+'" aria-label="Share this event">'+SHARE_SVG+'<span>Share</span></button>' : '';
      ...
      return '<div class="ev-layout">'+
          ...
          '<div class="ev-col-main">'+
            ...
            (eid ? '<div class="ev-playbar">'+((ev.artists||[]).length?'<button class="ev-bigplay" id="ev-playbill" type="button" aria-label="Play the line-up"><svg viewBox="0 0 24 24" fill="currentColor" aria-hidden="true"><path d="M8 5v14l11-7z"/></svg></button>':'')+'<div class="ev-pb-acts">'+(isPast?evLog:evHeart)+evShare+'</div></div>' : '')+
