    ...
    function diaryTLHTML(diary, isMe){
      ...
      return '<div class="prof-tl">'+diary.map(function(d,i){
        ...
        var ev = d.event_id ? '<button class="dv-event" type="button" data-event-id="'+esc(String(d.event_id))+'">View event</button>' : '';
        ...
        return '<div class="row"><div class="spine"><div class="dot"></div>'+(i<diary.length-1?'<div class="ln"></div>':'')+'</div><div class="card">'+del+'<div class="vn">'+vn+(ev?' <span class="dv-sep">·</span> '+ev:'')+'</div>'+(dl?'<div class="mt">'+esc(dl)+'</div>':'')+tiles+(d.note?'<div class="nt">'+esc(d.note)+'</div>':'')+'</div></div>';
      }).join('')+'</div>';
    }
    ...
    function pbStubHTML(d){
      ...
      return '<div class="pb-ds"'+(d.venue?' data-venue="'+esc(d.venue)+'"':'')+'><div class="pb-ds-date"><div class="pb-ds-d">'+day+'</div><div class="pb-ds-m">'+esc(mon)+'</div></div><div class="pb-ds-body"><div class="pb-ds-venue">'+(d.venue?esc(d.venue):'Gig')+evl+'</div><div class="pb-ds-line">'+esc(names||'—')+'</div></div></div>';
    }
    function pbDiaryPanel(p){
      ...
      var prompt=isMe?'<button class="pb-prob" id="pb-quicklog" type="button"><div class="pb-nudge-ic"><i class="ti ti-map-pin" aria-hidden="true"></i></div><div class="pb-prob-tx"><b>Been to a gig?</b><p>Add the night — get recommendations of similar artists.</p></div><span class="pb-prob-btn">Log a gig</span></button>':'';
    ...
    function pbShareCard(p){
      // Copy the profile link + toast — reliable everywhere, no flaky share-sheet.
      var url='https://soundcheck.club/u/'+encodeURIComponent(p.handle);
      function ok(){ showToast('Card link copied — soundcheck.club/u/'+p.handle); }
      if(navigator.clipboard&&navigator.clipboard.writeText){ navigator.clipboard.writeText(url).then(ok).catch(function(){ showToast(url); }); }
      else { showToast(url); }
    }
    function pbProfileHTML(p){
      ...
      return '<div class="pb-root">'+
        '<div class="pb-cover"><div class="pb-cover-actions"><button class="pb-ghost" id="pb-followers" type="button"><i class="ti ti-users" aria-hidden="true"></i> '+(p.followers||0)+' followers</button>'+actions+'</div></div>'+
        '<div class="pb-hero">'+tasteCardHTML(p)+
          '<div class="pb-sharebar"><button class="pb-share" id="pb-share" type="button"><span class="pb-ic"><i class="ti ti-share-2" aria-hidden="true"></i></span><span class="pb-share-t">'+shareLabel+'</span></button></div>'+
          (isMe?pbNudgeHTML(p):'')+
        '</div>'+
    ...
    function wirePbProfile(p){
      ...
      function wirePanel(key){
        ...
        panels.querySelectorAll('.pb-ds-ev[data-event-id]').forEach(function(b){ b.addEventListener('click',function(e){ e.stopPropagation(); if(b.dataset.eventId) openEvent(b.dataset.eventId); }); });
    ...
    async function openArtist(slug){
      ...
      try{
        ...
        // Festivals played — curated major-festival matches from past events, each
        // with a link to find that set on YouTube + the event page.
        if((a.festivals||[]).length){
          h+='<div class="sec-h" style="margin-top:26px">Festivals played</div><ul class="gig-history fest-list">'+a.festivals.map(function(f){
            var yr=f.event_date?String(f.event_date).slice(0,4):'';
            var yt='https://www.youtube.com/results?search_query='+encodeURIComponent((a.name||slug)+' '+f.name+' set');
            return '<li class="gh-row"><span class="gh-date">'+esc(yr)+'</span>'+
              '<button class="gh-venue" type="button" data-open-evt="'+esc(String(f.id))+'">'+esc(f.name)+(f.city?' <small>'+esc(cityLabel(f.city))+'</small>':'')+'</button>'+
              '<a class="gh-yt" href="'+yt+'" target="_blank" rel="noopener noreferrer" title="Find this set on YouTube"><svg viewBox="0 0 24 24" fill="currentColor" aria-hidden="true" style="width:14px;height:14px;vertical-align:-2px"><path d="M8 5v14l11-7z"/></svg> Set</a></li>';
          }).join('')+'</ul>';
        }
